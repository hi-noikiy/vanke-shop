...
class MySendMail {
    ...
    */
    protected function sendCommand($command, $code) {
        log::record4inter("邮件交互数据：".'Send command:' . $command . ',expected code:' . $code . '<br />', log::MOBILE_MESSAGE);
        //发送命令给服务器
        try{
            if(socket_write($this->_socket, $command, strlen($command))){
                ...
                $data = trim(socket_read($this->_socket, 1024));
                 log::record4inter("邮件交互数据：".'response:' . $data . '<br /><br />', log::MOBILE_MESSAGE);
                if($data) {
                    $pattern = "/^".$code."/";
    ...
    */
    private function socketSecurity() {
        ...
        log::record4inter("邮件建立连接：".$str, log::MOBILE_MESSAGE);
        if(!strpos($str, "220")){
            $this->_errorMessage = $str;
            return false;
        }
    ...
    */
    ...
    public function send_sys_email1($email_to, $subject, $message){
                $this->setServer(C('email_host'), C('email_id'), C('email_pass'), C('email_port'), true); //设置smtp服务器，到服务器的ssl连接
                $this->setFrom(C('email_addr')); //设置发件人
                $this->setReceiver($email_to); //设置收件人，多个收件人，调用多次
                $this->setMail($subject, $message); //设置邮件主题、内容
                $result = $this->sendMail(); //发送
		return $result;
	}
...
//                $mail = new PHPMailer;
//                $mail->SMTPDebug = 3;                               // debug模式 
//                $mail->isSMTP();                                      // SMTP
//                $mail->Host = C('email_host');  // 服务器地址
//                $mail->SMTPAuth = true;                    // 是否开始smtp服务
//                $mail->Username = C('email_id');             //用户名
//                $mail->Password =  C('email_pass');                       // 密码
//                $mail->SMTPSecure = 'tls';                            // 安全连接
//                $mail->Port = C('email_port');                                    // 端口号
//                $mail->setFrom(C('email_addr'), 's-vkwycgpt');//发件人邮箱地址
//                $mail->addAddress($email_to);     // 收件人邮箱地址
//                $mail->addReplyTo(C('email_addr'), 's-vkwycgpt');
