...
class UploadFile{
	 ...
	 * 允许上传的文件类型
	 */
	private $allow_type=array('jpg','jpeg','bmp','png','swf','tbi','pdf');
	/**
	 * 允许的最大文件大小，单位为KB
	 */
	...
	public function upfile($field){
		...
		if($this->upload_file['size'] > $this->max_size*5120){
...
//		$this->setErrorFileName($this->upload_file['tmp_name']);
	 ...
	 * @return bool
	 */
	public function upfile_all($field){
...
//		$this->setErrorFileName($this->upload_file['tmp_name']);
        ...
        public function upfile_exe($field){
		...
		if($this->upload_file['size'] > $this->max_size*2048){
			$error = Language::get('upload_file_size_cant_over').$this->max_size.'KB';
			$this->setError($error);
			return false;
		}
