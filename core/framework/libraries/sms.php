    ...
    public function send($mobile,$content) {
		...
		if($mobile_host_type==2)
		{
			return $this->mysend_yunpian($mobile,$content);
                ...
                {
                        return $this->mysend_smslt($mobile, $content);
                }
	...
	*/
    private function mysend_smsbao($mobile,$content){
	   ...
	   $url="http://api.smsbao.com/sms?u=".$user_id."&p=".$pass."&m=".$mobile."&c=".$content."";
           log::record4inter("短信宝发送的数据:".$url, log::MOBILE_MESSAGE);
 	   $res = file_get_contents($url);
           log::record4inter("短信宝返回的数据:".$res, log::MOBILE_MESSAGE);
 	   //return $res;
 	   $ok=$res=="0";
 	   if($ok)
 	   {
    ...
    private function mysend_yunpian($mobile,$content) {
         ...
         * @return type
         */
       private function mysend_smslt($mobile,$content){
           
           
           $SpCode = urlencode(C("mobile_spcode"));                  //企业编号
           $LoginName =urlencode(C("mobile_username"));              //登陆用户名
           $Password  = urlencode(C("mobile_pwd"));                  //密码
           $SerialNumber = time().time();                            //流水号  必须20位数
           $ScheduleTime = "";                                       //发送时间  空时 立即发送
           //$ExtendAccessNum = "";                                  //接入号扩展号
           $f = "1";                                                 //提交时检测方式，1为有效号码发送，无效号码不发送 非1 为 有无效号码存在时 都不发送
           //判定必要参数是否有值
           if($SpCode==""||$LoginName==""||$Password==""||$mobile==""||$content==""){
               return false;
           }
//           $mobile=urlencode($mobile);
//           $content=urlencode($content);
           $url= "http://sms.api.ums86.com:8899/sms/Api/Send.do?SpCode=".$SpCode."&LoginName=".
                   $LoginName."&Password=".$Password."&MessageContent=".$content."&UserNumber=".$mobile.
                   "&SerialNumber=".$SerialNumber."&ScheduleTime=".$ScheduleTime."&f=".$f;
           log::record4inter("一信通发送的数据:".$url, log::MOBILE_MESSAGE);
           //转换编码为gbk  短信要求使用gbk
           $url=mb_convert_encoding($url, "GBK","utf-8");
           $res = file_get_contents($url);
           //转换编码为utf-8 商城页面使用utf-8
           $res_utf=mb_convert_encoding($res,"utf-8" ,"GBK");//返回格式："result=0&description=发送短信成功"
           log::record4inter("一信通返回的数据:".$res_utf, log::MOBILE_MESSAGE);
 	   $msg_array = explode('&',$res_utf);
           $msg_result = explode('=',$msg_array['0']);
 	   if($msg_result['1']=="0")
 	   {
 	     return true;
 	   }
