 ...
 * @return obj 对象形式的返回结果
 */
function Factory($model = null, $base_path = null){
    static $_cache = array();
    $cache_key = $model.'.'.$base_path;
    if (!is_null($model) && isset($_cache[$cache_key])) return $_cache[$cache_key];
    $base_path = $base_path == null ? BASE_DATA_PATH : $base_path;
    $file_name = $base_path.'/factory/'.$model.'.factory.php';
    $class_name = $model.'Factory';
    if (!file_exists($file_name)){
        return $_cache[$cache_key] =  new Model($model);
    }else{
        require_once($file_name);
...
function ncPriceFormat($price) {
    ...
    if(substr($price_format, -2) == 00){
        $price_format_new	= number_format($price,2,'.','');
    }else{
        $price_format_new	= number_format($price,4,'.','');
    }
	return $price_format_new;
}
...
//    $site_url = empty($site_url) ? SHOP_SITE_URL : $site_url;
    ...
    $reg_match_from = array(
		...
		'/^article-showPurchaseDetail-article_id-(\d+)\.html$/',
...
//    }
    ...
    // 默认标志为不开启伪静态
    if (($act == 'search') && ($op == 'index') && (0 < $args['cate_id']) && empty($args['keyword'])) {
        $id = intval($args['cate_id']);
        $channel_list = rkcache('channel', true);
        if (0 < $channel_list[$id]) {
            $act = 'channel';
            $args = array();
