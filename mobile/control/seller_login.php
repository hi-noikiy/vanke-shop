    ...
    public function indexOp(){
        ...
        $member_info = $model_member->getMemberInfo(
            array(
                ...
                'member_passwd' => md5($_POST['password'])   //cary_fix
            )
        );
        if(!$member_info) {
        ...
        if($token) {
            output_data(array('seller_name' => $seller_info['seller_name'], 'store_name' => $store_info['store_name'], 'store_id' => $seller_info['store_id'] , 'key' => $token));
        } else {
            output_error('登录失败');
        }
    }
