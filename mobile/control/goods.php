...
class goodsControl extends mobileHomeControl{
    ...
    public function goods_listOp() {
        ...
        $fieldstr .= ',is_virtual,is_presell,is_fcode,have_gift,goods_jingle,store_id,store_name,is_own_shop,goods_third_price';
        ...
        foreach($goods_list as $key=>$rows){
            $goods_list[$key]['goods_price']  = $rows['goods_third_price'];
        }
    ...
    public function goods_detailOp() {
        ...
        if ($member_id = $this->getMemberIdIfExists()) {
            ...
            if (!$goods_detail['goods_info']['is_virtual']) {
                // 店铺优惠券
                $condition = array();
                $condition['voucher_t_gettype'] = 3;   //cary_fix  数据库中无相应字段
