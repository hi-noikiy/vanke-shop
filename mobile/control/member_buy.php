...
//            $pay['member_available_pd'] = $this->member_info['available_predeposit'];
     ...
     * 判断商品购买数量
     */
    public function check_numOp(){
        if(!empty($_POST['cart_id'])){
            $model = Model();
            //获取goods_id组装goodsid
            $a = strstr($_POST['cart_id'],",");
            $goods_data = explode(',', htmlspecialchars($_POST['cart_id']));
            foreach($goods_data as $rows){
                $goods[] = explode('|', $rows);
            }
            if(!$a){
                //判断是购物车 还是 商品表
                $goods_where_stats['cart_id'] = $goods[0][0];
                $goods_stats = $model->table('cart')->where($goods_where_stats)->find();
            }
            if(is_array($goods)){
                $re_msg = array();
                //当接收到的是多个的商品ID时执行
                ...
                foreach($goods as $g_goods){
                    if($goods_stats || $a != false){
                        $where['cart_id'] = $g_goods[0];
                        $goods_num = $model->table('cart,goods')->join('left join')->on('cart.goods_id = goods.goods_id')->where($where)->field('goods.min_num,goods.max_num,goods.goods_id')->find();
                    }else{
                        $where['goods_id'] = $g_goods[0];
                        $goods_num = $model->table('goods')->where($where)->field('min_num,max_num,goods_id')->find();
                    }
                    if($goods_num){
                        $re_msg[$i]['goods_id'] = $goods_num['goods_id'];
                        $re_msg[$i]['state'] = 2;
