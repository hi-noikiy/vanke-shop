		...
		foreach ($cart_list as $key => $val) {
			...
			if($goods_data['goods_promotion_type']){
				$cart_a[$val['store_id']]['goods'][$key]['goods_price'] = $goods_data['goods_promotion_price'];
                        ...
                        if($_SESSION['identity'] == MEMBER_IDENTITY_ONE){
                            //这是采购
                            $val['goods_price'] = $goods_data['goods_price'];
                            $cart_a[$val['store_id']]['goods'][$key]['goods_price'] = $goods_data['goods_price'];
                            ...
                            //这是第三方
                            $val['goods_price'] = $goods_data['goods_third_price'];
                            $cart_a[$val['store_id']]['goods'][$key]['goods_price'] = $goods_data['goods_third_price'];
                        }else {
                            $val['goods_price'] = $goods_data['goods_third_price'];
                            $cart_a[$val['store_id']]['goods'][$key]['goods_price'] = $goods_data['goods_third_price'];
                        }
...
//        $logic_buy_1->getGroupbuyInfoWap($goods_info,$this->member_info['member_id']);
        //add cary  团购购买限制检测
        if(empty($goods_info)){
            output_error('商品超出购买限制');
        }
        ...
        $param['goods_price'] = $goods_info['goods_third_price'];
        $param['goods_image'] = $goods_info['goods_image'];
        $param['store_name'] = $goods_info['store_name'];

        $model = Model('cart');
        $cart_have_where['buyer_id'] = $param['buyer_id'];
        $cart_have_where['goods_id'] = $param['goods_id'];
        $cart_have = $model->where($cart_have_where)->field('cart_id,goods_num')->find();
        if($cart_have['cart_id'] > 0){
            //加1
            $updata_insert_data['goods_num'] = $cart_have['goods_num'] + $quantity;
            $result = $model->where($cart_have_where)->update($updata_insert_data);
        }else{
            //新增
    ...
    private function _check_goods_storage(& $cart_info, $quantity, $member_id) {
		...
		if ($cart_info['bl_id'] == '0') {
            ...
            //团购
            $logic_buy_1->getGroupbuyInfo($goods_info);
//            $logic_buy_1->getGroupbuyInfoWap($goods_info,$member_id);
