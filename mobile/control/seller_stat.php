...
class seller_statControl extends mobileSellerControl {
    ...
    private $colors = array('#7a96a4','#cba952','#667b16','#a26642','#349898','#c04f51','#5c315e','#445a2b','#adae50','#14638a','#b56367','#a399bb','#070dfa','#47ff07','#f809b7');
    ...
    public function __construct(){
        ...
        $this->search_arr['stime'] = strtotime($this->search_arr['starttime']);
        $this->search_arr['etime'] = strtotime($this->search_arr['endtime']);
    ...
    public function goodsflowOp(){
        ...
        //cary_fix  当销售商品种类少于10的情况
        $count_size = sizeof($statlist_tmp);
        $count_total = $count_size >= $topnum ? $topnum : $count_size;
        //构造横轴数据
...
//        for($i=1; $i<=$topnum; $i++){
        ...
        foreach ((array)$statlist as $k=>$v){
            ...
            if ($province_id){
                //数据
                $y_data1[] = array('name'=>strval($province_array[$province_id]),'y'=>$v[$data_type], 'color'=> $this->colors[$k]);
                //横轴
                //$x_data[] = strval($province_array[$province_id]);
            } else {
                //数据
                $y_data1[] = array('name'=>'未知','y'=>$v[$data_type], 'color'=> $this->colors[$k]);
                //横轴
                //$x_data[] = '未知';
            }
        }
        output_data(array('x'=>$x_data, 'y'=>$y_data1));
    }
}
