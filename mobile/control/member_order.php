    ...
    public function order_listOp() {
		...
		$condition['buyer_id'] = $this->member_info['member_id'];
                if($_POST['order_key']){
                    ...
                    $order_goods_where['buyer_id'] = $_SESSION['member_id'];
                    $order_goods_where['goods_name']  = array('like','%'.$_POST['order_key'].'%');
                    $order_goods_list = $model_order_goods->where($order_goods_where)->field('order_id')->select();
                    if($order_goods_list){
                        //如果查询到了就拼接查询语句
                        foreach($order_goods_list as $or_c_g){
                            $order_id_c .= $or_c_g['order_id'].',';
                        }
                        $condition['order_id'] = array('in',$order_id_c);
                    }else{
                        //如果标题内没有则查询订单
                        $condition['order_sn'] = htmlspecialchars($_POST['order_key']);
                    }
                }
