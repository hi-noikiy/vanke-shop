...
$(function() {
    ...
    $("#ToBuyStep2").click(function() {
        ...
        //请求验证判断商品限购数量大小
        $.post(
            ApiUrl + '/index.php?act=member_buy&op=check_num',
            ...
            function(e){
                ...
                var send_two = 1;
                var if_stop = 1;
                $.each(e.datas, function (i, item) {
                    var span_msg = $("[lang='"+item.goods_id+"']").html();
                    var if_span = $("[lang='"+item.goods_id+"']").find('span').html();
                    var is_unde = 2;
                    if(if_span != null){
                        is_unde = 1;
                        span_msg = $("[lang='"+item.goods_id+"']").span().html();
                    }
                    sumreturn += 1;
                    if(item.state == 1){
                        send_two += 1;
                        ...
                        if(item.goods_max >= 0 && a != 2){
                            lang += '　　( 当前商品最大购买数量为'+item.goods_max+' )';
                        }
                        if(is_unde != 1){
                            lang += '</span>';
                            $("[lang='"+item.goods_id+"']").html(lang);
                        }else{
                            $("[lang='"+item.goods_id+"']").find('span').html(lang);
                        }
                        $("[lang='"+item.goods_id+"']").parent().next().find('em').css("color",'red') ;
                        if_stop = 2;
                    }
                });
                if(if_stop != 2){  
