...
//        $condition['order_lei']  = array('neq',2);
    ...
    public function statistics_orders(){
    	...
    	if (empty($ordergoods_count)) {
    		...
    		$order_list_rows = $model->table('order')->field($field_orders)->where("add_time > $where_count")->limit('200')->select();//->where(array('order_id'=>array('in',$orderid_arr)))->select();
    	}
    	...
    	foreach ($order_list_rows as $k=>$v){
    		...
    		foreach ($order_goods_row as $order=>$goods){
    			...
    			if ($v['order_state'] == 40 or $v['order_state'] == 30 or $v['order_state'] == 14 or $v['order_state'] == 31 or $v['order_state'] == 33 or $v['order_state'] == 50) {
