  ...
  <form id="admin_form" method="post" action='index.php?act=admin&op=admin_edit&admin_id=<?php echo $output['admininfo']['admin_id'];?>'>
    ...
    <table class="table tb-type2">
      <tbody>
        ...
        <tr>
          <td colspan="2" class="required"><label class="validation" for="gadmin_name"><?php echo $lang['city_name'];?>:</label></td>
        </tr>
        <tr class="noborder">
          <td class="vatop rowform">
          <select name="city_id">
          <?php foreach((array)$output['city'] as $v){?>
          <option  <?php if($v['id'] == $output['admininfo']['city_id']){ echo "selected='selected'";} ?> value="<?php echo $v['id'];?>"><?php echo $v['city_name'];?></option>
          <?php }?>
          </select>
          </td>
          <td class="vatop tips"><?php echo $lang['admin_add_gid_tip_1'];?></td>
        </tr>
      </tbody>
      <tfoot>
        <tr class="tfoot">
