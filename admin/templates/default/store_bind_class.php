...
<!--        <li><a href="index.php?act=store&op=store_bind_class_applay_list" ><span>经营类目申请</span></a></li>-->
        <li><a href="JavaScript:void(0);" class="current"><span>佣金设置</span></a></li>
      </ul>
    </div>
  </div>
  ...
  <table class="table tb-type2" >
    <thead class="thead">
      <tr class="space">
        <th colspan="15"><span>重新绑定分类</span></th>
      </tr>
    </thead>
    <tbody>
      <tr class="noborder">
        <td class="required" colspan="2" >该操作会删除旧分类, 重新添加分类</td>
      </tr>
      <tr class="noborder">
          <td class="vatop rowform">
              <form id="init_form" action="<?php echo urlAdmin('store', 'store_bind_init');?>" method="post">
                  <input name="store_id" type="hidden" value="<?php echo $output['store_info']['store_id'];?>">
              默认佣金: <input id="new_commis_rate" name="new_commis_rate" class="w60" type="text" value="" /> %
    ...
    <tfoot >
      <tr>
        <td colspan="2"><a id="btn_bind_init" class="btn" href="JavaScript:void(0);" /><span>确认</span></a></td>
      </tr>
    </tfoot>
  </table>
</div>
...
$(document).ready(function(){
    ...
    $('#btn_bind_init').on('click', function() {
        $('#error_message2').hide();
        var commis_rate = parseInt($('#new_commis_rate').val(), 10);
        if(isNaN(commis_rate) || commis_rate < 0 || commis_rate > 100) {
            $('#error_message2').text('请填写正确的分佣比例');
            $('#error_message2').show();
            return false;
        }


        $.post('<?php echo urlAdmin('store', 'store_bind_init');?>', {store_id: <?php echo $output['store_info']['store_id'];?>, new_commis_rate:commis_rate  }, function(data) {
            if(data.result) {
