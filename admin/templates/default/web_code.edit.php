  ...
  <form id="upload_tit_form" name="upload_tit_form" enctype="multipart/form-data" method="post" action="index.php?act=web_api&op=upload_pic" target="upload_pic">
    ...
    <table id="upload_tit_type_txt" class="table tb-type2" <?php if($output['code_tit']['code_info']['type'] != 'txt'){ ?>style="display:none;"<?php } ?>>
      <tbody>
        ...
        <tr>
          <td colspan="2" class="required"><?php echo '分类编号'.$lang['nc_colon'];?></td>
        </tr>
        <tr class="noborder">
          <td class="vatop rowform">
            <input class="txt" type="text" name="tit[classid]" id="tit_classid" value="<?php echo $output['code_tit']['code_info']['classid'];?>">
          </td>
          <td class="vatop tips"><?php echo '如填写楼层对应的分类编号。';?></td>
        </tr>
      </tbody>
    </table>
  ...
  <form id="recommend_list_form">
    ...
    <?php foreach ($output['code_recommend_list']['code_info'] as $key => $val) { ?>
            <dl select_recommend_id="<?php echo $key;?>">
              ...
              <h4 class="dialog-handle-title">栏目编号</h4>
              <div class="dialog-handle-box"><span class="left">
                  <input name="recommend_list[<?php echo $key;?>][recommend][catid]" value="<?php echo $val['recommend']['catid'];?>" type="text" class="w100">
                  </span><span class="right">请填写栏目编号,格式为: 数字,可以为空</span>
                <div class="clear"></div>
              </div>
              </dt>
              <dd>
