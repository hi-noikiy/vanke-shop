  ...
  <div class="fixed-bar">
    <div class="item-title">
      ...
      <ul class="tab-base">
        ...
        <li><a href="<?php echo urlAdmin('goods', 'goods', array('type' => 'waitverify'));?>"><span>上架等待审核</span></a></li>
<!--        <li><a href="<?php echo urlAdmin('goods', 'goods_set');?>"><span><?php echo $lang['nc_goods_set'];?></span></a></li>-->
          <li><a href="<?php echo urlAdmin('goods', 'goods', array('type' => 'off'));?>"><span>下架审核</span></a></li>
  ...
  <form method="get" name="formSearch" id="formSearch">
    ...
    <table class="tb-type1 noborder search">
      ...
      <!--时间-->
         <th><label for="query_start_time">发布时间</label></th>
      ...
      <tr>
        <td><ul>
            ...
            <li><?php echo $lang['goods_index_help2'];?></li>
                <li>商品佣金，必须设置截止时间，商品佣金才会生效，超过截止时间后恢复系统默认佣金</li>
          </ul></td>
      </tr>
    </tbody>
  </table>
...
<!--          <th class="w72 align-center">价格(元)</th>
          <th class="w72 align-center">市场价/第3方价/协议价(元)</th>-->
            <th class="w72 align-center">总库存</th>
          <th class="w72 align-center">商品状态</th>
          <th class="w72 align-center">审核状态</th>
            <th class="w72 align-center">商品佣金</th>
            <th class="w108 align-center">佣金截至</th>
      ...
      <tbody>
        ...
        <tr class="hover edit">
         ...
         <!-- <td><i class="icon-plus-sign" style="cursor: pointer;" nctype="ajaxGoodsList" data-comminid="<?php /*echo $v['goods_commonid'];*/?>" title="点击展开查看此商品全部规格；规格值过多时请横向拖动区域内的滚动条进行浏览。"></i></td>-->
          <td><a href="javascript:void(0);" onclick="look_list('<?php echo $v['goods_commonid'];?>')">
                  <i class="icon-plus-sign" style="cursor: pointer;" data-comminid="<?php /*echo $v['goods_commonid'];*/?>" title="点击展开查看此商品全部规格；规格值过多时请横向拖动区域内的滚动条进行浏览。"></i>
              </a></td>
          ...
          <td class="align-center"><?php /*echo $v['goods_costprice']*/?>/<?php /*echo $v['goods_third_price']*/?>/<?php /*echo $v['goods_marketprice']*/?></td>-->
          ...
          <td class="align-center sort"><span nc_type="commis_rate" column_id="<?php echo $v['goods_commonid'];?>" title="<?php echo $lang['nc_editable'];?>" class="editable " style="vertical-align: middle; margin-right: 4px;"><?php echo $v['goods_commis_rate'];?></span>%
          </td>
            <td class="align-center ">
                <p><a href="#" class="endtime" data-type="date" data-pk="<?php echo $v['goods_commonid'];?>" data-url="<?php echo ADMIN_SITE_URL;?>/index.php?act=goods&op=goods_commis_time_update" data-title="请选择佣金截止时间"><?php echo $v['goods_commis_end'] == 0 ? '' : @date('Y-m-d',$v['goods_commis_end']);?></a></p>
            </td>
          <td class="align-center"><a href="<?php echo urlShop('goods', 'index', array('goods_id' => $output['storage_array'][$v['goods_commonid']]['goods_id']));?>" target="_blank"><?php echo $lang['nc_view'];?></a><?php if($output['verify'][$v['goods_verify']] == "通过" && $output['state'][$v['goods_state']] != "违规下架"){?> &nbsp;|&nbsp;<a href="javascript:void(0);" onclick="goods_lockup(<?php echo $v['goods_commonid'];?>);">违规下架</a><?php }?></td>
        </tr>
...
function look_list(id){
    layui.use('layer', function(){
        ...
        layer.open({
            ...
            resize: false,
            area: ['1250px', '580px'],
            content: '/admin/index.php?act=goods&op=goodsSupList&commonid='+id,
        });
    });
}
$(function(){
                ...
                $.getJSON('index.php?act=goods&op=get_goods_list_ajax' , {commonid : _commonid}, function(date){
                    if (date != 'false') {
                        ...
                        $.each(date, function(i, o){
                            $('<li><div class="goods-thumb" title="商家货号：' + o.goods_serial + '"><a href="' + o.url + '" target="_blank"><image src="' + o.goods_image + '" ></a></div>' + o.goods_spec + '<div class="goods-price">外部物料编号：<br/><em style="width:100px;" title="' + o.goods_serial + '">' + o.goods_serial + '</em></div><div class="goods-price">价格：<em title="￥' + o.goods_price + '">￥' + o.goods_price + '</em></div><div class="goods-storage">库存：<em title="' + o.goods_storage + '">' + o.goods_storage + '</em></div><a href="' + o.url + '" target="_blank" class="ncsc-btn-mini">查看商品详情</a></li>').appendTo(_ul);
                            });
                        _ul.appendTo(_div);
                        _parenttr.next().show();
                        // 计算div的宽度
...
// 修改分佣比例
$('span[nc_type="commis_rate"]').inline_edit({act: 'goods',op: 'goods_commis_rate_update'});
</script>
