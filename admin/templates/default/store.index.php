...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title">
      ...
      <ul class="tab-base">
        <!--<li><a href="JavaScript:void(0);" ><span><?php echo $lang['manage'];?></span></a></li>-->
		<li><a href="index.php?act=store&op=store_joinin2" class="current"><span>认证申请审核</span></a></li>
  ...
  <tbody>
    <tr><th><label><?php echo $lang['belongs_level'];?></label></th>
      <td><select name="grade_id">
          ...
          <?php } ?>
          <?php } ?>
        </select></td><th><label for="owner_and_name">供应商账号</label></th>
      <td><input type="text" value="<?php echo $output['owner_and_name'];?>" name="owner_and_name" id="owner_and_name" class="txt"></td><td></td><th><label>供应商类型</label></th>
        <td>
    ...
    <table class="table tb-type2">
      ...
      <tbody>
        ...
        <td class="align-center w220">
            <a href="index.php?act=store&op=store_joinin_detail&member_id=<?php echo $v['member_id'];?>&city=<?php echo $v['first_city_id'];?>">查看</a>&nbsp;&nbsp;<a href="index.php?act=store&op=store_edit&store_id=<?php echo $v['store_id']?>"><?php echo $lang['nc_edit'];?></a>&nbsp;&nbsp;
            <a href="index.php?act=store&op=store_bind_class&store_id=<?php echo $v['store_id']?>">佣金</a>
			<!--
            <?php if (getStoreStateClassName($v) != 'open' && cookie('remindRenewal'.$v['store_id']) == null) {?><a href="<?php echo urlAdmin('store', 'remind_renewal', array('store_id'=>$v['store_id']));?>">提醒续费</a><?php }?>&nbsp;&nbsp; <a href="index.php?act=store&op=del&id=<?php echo $v['store_id']?>&member_id=<?php echo $v['member_id']?>" onclick="return confirm('您确认要删除此供应商吗？');">删除</a>
             -->
        </td>
        </tr>
        <?php } ?>
