...
<script type="text/javascript">
  function checkClassIsDel( id ){//00表示不能删除，-1表示这个分类不存在，01表示可以删除
    var isDel = false;
    $.ajax({
      url: 'index.php?act=goods_class&op=goods_class_check&gc_id='+id,
      dataType: 'json',
      success: function(data){
        if(data['isDel'] ==='01'){
            if(confirm('<?php echo $lang['goods_class_index_ensure_del'];?>'))
                window.location = 'index.php?act=goods_class&op=goods_class_del&gc_id='+id;
        }else if( data['isDel'] ==='00'){//当前分类不能删除
          alert(data['msg']);
        }else if( data['isDel'] ==='01'){//当前分类不能删除
          alert(data['msg']);
        }
      },
      error: function(){
  ...
  function pushGoodsClassToVS( ) {//00表示推送成功，-1表示推送失败
    var code = false;
    $.ajax({
      url: 'index.php?act=goods_class&op=pushGoodsClassToVS',
      dataType: 'json',
      success: function (data) {
        if (data['resultCode'] === '0') {//当前分类不能删除
          alert(data['resultMsg']);
        } else if (data['resultCode'] === '-1') {//当前分类不能删除
          alert(data['resultMsg']);
        }
      },
      error: function () {
    ...
    <table class="table tb-type2">
      ...
      <tbody>
        ...
        <tr class="hover edit">
          ...
          <td class="w96">
              <a href="index.php?act=goods_class&op=nav_edit&gc_id=<?php echo $v['gc_id'];?>">设置</a>
              | <a href="index.php?act=goods_class&op=goods_class_edit&gc_id=<?php echo $v['gc_id'];?>"><?php echo $lang['nc_edit'];?></a>
              | <a   style="cursor:pointer" onclick="checkClassIsDel(<?php echo $v['gc_id'];?>);"><?php echo $lang['nc_del'];?></a>
          </td>
        </tr>
        <?php } ?>
        <?php }else { ?>
