	...
	public function code_updateOp() {
		...
		if (!empty($code)) {
			...
			$var_name = $code['var_name'];
                        foreach($_POST[$var_name] as $k1 => $v){
                            foreach($_POST[$var_name][$k1]['goods_list'] as $k => $v){
                                //获取城市中心
//                                $citydata=Model()->query("select s.store_city_name from sc_goods as g "
//                                        . "left join sc_store s on g.store_id=s.store_id where g.goods_id='".$v['goods_id']."'");
                                $model = Model();
                                $store_city_id = $model->table('goods,store')->join('left')->on('goods.store_id=store.store_id')
                                        ->field('store_city_id,g_costprice')
                                        ->where('goods.goods_id='.$v['goods_id'])
                                        ->select();
                                $cache_news = new CacheFile();
                                $c_city = $cache_news->get('city_name');
                                $city_id = explode(',',$store_city_id[0]['store_city_id']);
                                if($c_city){
                                    foreach($city_id as $c_id){
                                        foreach($c_city as $ca_id){
                                            if($c_id == $ca_id['id']){
                                                $store_city_name .= $ca_id['city_name'].',';
                                            }
                                        }
                                    }
                                }
                                $_POST[$var_name][$k1]['goods_list'][$k]['store_city_name']=  substr($store_city_name, 0,-1);
                                $_POST[$var_name][$k1]['goods_list'][$k]['g_costprice']=  $store_city_id[0]['g_costprice'];
                                unset($store_city_name);
                            }
                        }
