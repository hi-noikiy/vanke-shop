...
class dbControl extends SystemControl{
	...
	public function dbOp(){
		...
		if ($_POST['form_submit'] == 'ok'){
			 ...
			 * 验证文件夹是否重复
			 */
			if (is_dir(BASE_ROOT_PATH.DS.'data/sql_back'.DS.$_POST['backup_name'])) {
	...
	public function db_restoreOp(){
		...
		if ($_POST['form_submit'] == 'ok'){
			if (!empty($_POST['dir_name']) && is_array($_POST['dir_name'])){
				$dir = BASE_ROOT_PATH.DS.'data/sql_back';
				foreach ($_POST['dir_name'] as $k => $v){
					if (file_exists(BASE_ROOT_PATH.DS.'data/sql_back'.DS.$v)){
						$file_list  = array();
		...
		$tmp_list = readDirList(BASE_ROOT_PATH.DS.'data/sql_back');
		/**
		 * 整理内容
		 */
		$dir_list = array();
		if (is_array($tmp_list)){
			foreach ($tmp_list as $k => $v){
				$dir_list[$k]['name'] = $v;
				$dir_list[$k]['make_time'] = date('Y-m-d H:i:s',filemtime(BASE_ROOT_PATH.DS.'data/sql_back'.DS.$v));
				$dir_list[$k]['size'] = number_format((getDirSize(BASE_ROOT_PATH.DS.'data/sql_back'.DS.$v)/1024),2).'KB';
				$dir_list[$k]['file_num'] = count(glob(BASE_ROOT_PATH.DS.'data/sql_back'.DS.$v.DS."*.sql"));
			}
		}
	...
	public function db_importOp(){
		$lang 	= Language::getLangContent();
		if ($_GET['dir_name'] != '' && file_exists(BASE_ROOT_PATH.DS.'data/sql_back'.DS.$_GET['dir_name'])){
			$model_db = Model('db');
	...
	public function db_delOp(){
		$lang 	= Language::getLangContent();
		if ($_GET['dir_name'] != '' && file_exists(BASE_ROOT_PATH.DS.'data/sql_back'.DS.$_GET['dir_name'])){
			$dir = BASE_ROOT_PATH.DS.'data/sql_back'.DS.$_GET['dir_name'];
			$file_list = array();
			 ...
			 * 删除目录
			 */
			@rmdir(BASE_ROOT_PATH.DS.'data/sql_back'.DS.$_GET['dir_name']);
	...
	public function ajaxOp(){
		switch ($_GET['branch']){
			 ...
			 */
			case 'db_file':
				if ($_GET['dir_name'] != '.' && $_GET['dir_name'] != '..' && is_dir(BASE_ROOT_PATH.DS.'data/sql_back'.DS.$_GET['dir_name'])){
					$dir = BASE_ROOT_PATH.DS.'data/sql_back'.DS.$_GET['dir_name'];
					$tmp = array();
