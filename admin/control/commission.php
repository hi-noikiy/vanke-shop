...
class commissionControl extends SystemControl{
    ...
    public function indexOp(){
        ...
        Tpl::showpage('bill_order_statis_commission.index');
    }
	...
	public function show_statisOp(){
		...
		if (!empty($_GET['os_month'])) {
		    $condition['oc_month'] = intval($_GET['os_month']);
		}
		if (is_numeric($_GET['bill_state'])) {
		    $condition['oc_state'] = intval($_GET['bill_state']);
		}
		if (preg_match('/^\d{1,8}$/',$_GET['query_store'])) {
			$condition['oc_store_id'] = $_GET['query_store'];
		}elseif ($_GET['query_store'] != ''){
			$condition['oc_store_name'] = $_GET['query_store'];
		}
		$bill_list = $model_bill->getOrderBillList($condition,'*',30,'oc_no desc');
		Tpl::output('bill_list',$bill_list);
		Tpl::output('show_page',$model_bill->showpage());
		...
		Tpl::showpage('bill_order_statis_commission.show');
	}
	...
	public function show_billOp(){
		if (!preg_match('/^2\d{5,12}$/',$_GET['ob_no'],$match)) {
			//showMessage('参数错误','','html','error');
		}
		$model_bill = Model('commission');
		$bill_info = $model_bill->getOrderBillInfo(array('oc_no'=>$_GET['ob_no']));
		if (!$bill_info){
			showMessage('参数错误','','html','error');
		}
		...
		$order_condition['store_id'] = $bill_info['oc_store_id'];
		...
		if ($if_start_date || $if_end_date) {
		    ...
		    $order_condition['finnshed_time'] = array('between',"{$bill_info['oc_start_date']},{$bill_info['oc_end_date']}");
		}
		if ($_GET['query_type'] == 'refund') {
			...
			$refund_condition['store_id'] = $bill_info['oc_store_id'];
		    ...
		    $cost_condition['cost_store_id'] = $bill_info['oc_store_id'];
		    ...
		    //取得店铺名字
		    $store_info = Model('store')->getStoreInfoByID($bill_info['oc_store_id']);
