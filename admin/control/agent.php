...
class agentControl extends SystemControl{
	...
	public function __construct(){
		parent::__construct();
		Language::read('agent,agent_grade');
	}
    ...
    public function agentOp(){
        $condition_str = '';
        if(!empty($_GET['agent_id']) && intval($_GET['agent_id']) > 0 ) {
            $condition_str .= " and agent.seller_id = '". $_GET['agent_id'] ."'";
        }
        if(!empty($_GET['agent_name'])) {
            $condition_str .= " and agent.seller_name like '%". $_GET['agent_name'] ."%'";
        }
        if(!empty($_GET['grade_id']) && intval($_GET['grade_id']) > 0) {
            $condition_str .= " and agent.grade_id = '". $_GET['grade_id'] ."'";
        }
        ...
        $agent_list = $model_agent->getAgentInfoList($condition_str, $page, 'agent.seller_id desc');
	 ...
	 * 店铺 待审核列表
	 */
	public function agent_joininOp(){
		//店铺列表
	 ...
	 * 审核详细页
	 */
	public function agent_joinin_detailOp(){
		$model_store_joinin = Model('agent_joinin');
        ...
        if (!empty($joinin_detail['sg_info'])) {
            $store_grade_info = Model('agent_grade')->getOneGrade($joinin_detail['sg_id']);
	 ...
	 * 审核
	 */
	public function agent_joinin_verifyOp() {
        $model_store_joinin = Model('agent_joinin');
    ...
    private function store_joinin_verify_pass($joinin_detail) {
        ...
        if ($param['paying_amount'] > 0) {
            showMessage('代理申请审核完成','index.php?act=agent&op=agent_joinin');
        } else {
    ...
    private function store_joinin_verify_open($joinin_detail) {
        ...
        if($_POST['verify_type'] === 'pass') {
            ...
            $seller_array['grade_id'] = $joinin_detail['sg_id']; //代理级别
			...
			if($state) {
                showMessage('代理开通成功','index.php?act=agent&op=agent_joinin');
            } else {
                showMessage('代理开通失败','index.php?act=agent&op=agent_joinin');
            }
        } else {
            showMessage('代理开通拒绝','index.php?act=agent&op=agent_joinin');
        }
    }
