...
class stat_tradeControl extends SystemControl{
    ...
    $model = Model('stat');
    	$url = "?act=stat_trade&op=income&page={page}&search_year=".$_GET["search_year"]."&search_month=".$_GET['search_month']; //分页地址，如果有检索条件 ="?page={page}&q=".$_GET['q']
    	...
    	if($_GET['exporttype'] == 'excel'){
			...
			$excel_data[0][2] = array('styleid'=>'s_title','data'=>'订单数量');
 			$excel_data[0][3] = array('styleid'=>'s_title','data'=>'订单金额');
			foreach ($bill_list as $k=>$v){
				$excel_data[$k+1][0] = array('data'=>$v['store_name']);
				$excel_data[$k+1][1] = array('data'=>$v['member_name']);
				$excel_data[$k+1][2] = array('data'=>$v['COUNT(stat_order.order_sn)']);
 				$excel_data[$k+1][3] = array('data'=>$v['SUM(order_amount)']);
			}
			$excel_data = $excel_obj->charset($excel_data,CHARSET);
			$excel_obj->addArray($excel_data);
 			$excel_obj->addWorksheet($excel_obj->charset('销量统计',CHARSET));
		    $excel_obj->generateXML($excel_obj->charset('销量统计',CHARSET).$year.'-'.$month.'-01');
			exit();
    	}else{
    		//一页显示的行数
    		$showrow = 6; 
	    	...
	    	//详情中的日期参数
	    	$date = substr($condition['os_month'],4,2);
	    	$curpage = empty($_GET['page']) ? 1 : $_GET['page'];
	    	$start_time = strtotime($condition['os_month'].'01');
	    	$end_time = date(strtotime("+1 months",$start_time))-1;
			$sql = "SELECT SUM(order_amount),stat_order.store_name,COUNT(stat_order.order_sn),stat_order.store_id,store.member_name 
	    	...
	    	$total = count($model->getBillList($condition,'ob'));
