    ...
    public function alipayOp() {
        ...
        if(!empty($detail_array)) {
            ...
            if ($refund_amount > 0) {
                ...
                if($b_date != date('Ymd')) {
                    if($refund_id < 100){
                        $batch_no='00'.$refund_id;
                    }
                    $parameter['batch_no']=$batch_no;
                    $batch_no = date('Ymd').$batch_no;//substr($batch_no, 8);//批次号。支付宝要求格式为：当天退款日期+流水号。
                ...
                $arr['service']             = 'refund_fastpay_by_platform_pwd';
                $arr['_input_charset']      = $parameter['_input_charset'];
                $arr['notify_url']          = $parameter['notify_url'];
                $arr['refund_date']         = date("Y-m-d H:i:s");
                $arr['batch_no']            = $parameter['batch_no'];
                $arr['batch_num']           = $parameter['batch_num'];
                $arr['detail_data']         = $parameter['detail_data'];
                ...
                $array_url = json_decode($returndata,true);
                if($array_url['process_status'] == 'success'){
                    $re_cpay_url = 'https://mapi.alipay.com/gateway.do?'.$array_url['process_ret'];
                    @header("Location: ".$re_cpay_url);
                }
                exit;
