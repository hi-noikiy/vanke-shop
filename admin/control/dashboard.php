...
class dashboardControl extends SystemControl{
	...
	public function welcomeOp(){
        ...
        //  c extension
        try {
            $r = new ReflectionExtension('mall');
            $statistics['php_version'] .= ' / ' . $r->getVersion();
        } catch (ReflectionException $ex) {
        }

        //获取数据
        $join_where = $admin_info['city_id'] == '0' ? "":"city_center = '".$admin_info['city_id']."' and ";
        $all_num_data = array(
            'num_a'=>$model->table('store_joinin')->where($join_where."joinin_state = '43'")->count(),
            'num_b'=>$model->table('store_joinin')->where($join_where."joinin_state = '44' and store_state = '34'")->count(),
            'num_c'=>$list_num,
            'num_d'=>$model->table('goods_common')->where("del = '1' and goods_state = '2'")->count(),
            'num_e'=>$model->table('goods_common')->where("del = '1' and goods_verify = '10'")->count(),
        );
        Tpl::output('suptimeend',$list_num);
        Tpl::output('allnum',$all_num_data);
