	...
	public function mobileOp(){
		...
		if (chksubmit()){
			...
			$update_array['mobile_memo'] 	= $_POST['mobile_memo'];
                        $update_array['mobile_spcode'] 	= $_POST['mobile_spcode'];
			$result = $model_setting->updateSetting($update_array);
			if ($result === true){
...
//			$result = $obj_email->send($email_test,$subject,$message);
                        ...
                        $mail = new MySendMail();
                        $mail->setServer($email_host, $email_id, $email_pass, $email_port, true); //设置smtp服务器，到服务器的ssl连接
                        $mail->setFrom($email_addr); //设置发件人
                        $mail->setReceiver($email_test); //设置收件人，多个收件人，调用多次
