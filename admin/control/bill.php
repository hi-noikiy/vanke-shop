		...
		if (preg_match('/^\d{1,8}$/',$_GET['query_store'])) {
			...
			$condition['ob_store_name'] = $_GET['query_store'];
                ...
                if($admininfo['cityid'] > 0){
                    ...
                    $store_list = $model_store->field('store_city_id,store_id')->select();
                    foreach($store_list as $rows){
                        @$id_st = explode(',',$rows['store_city_id']);
                        if(is_array($id_st)){
                            foreach($id_st as $i_rows){
                                if($i_rows == 1){
                    ...
                    $condition['ob_store_id'] = array('in',$store_id);  
                }
