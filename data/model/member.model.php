...
class memberModel extends Model {
    ...
    public function createSession($member_info = array(),$reg = false) {
		...
		$_SESSION['avatar'] 	= $member_info['member_avatar'];
                $_SESSION['city_id']    = $member_info['city_id'];
                //判断登录会员属性
                //会员状态：1 普通会员 2 采购员 3 认证为开店商家 4 认证开店商家
                $model = Model();
                
                $member_rolue_id = $model->table('member')->field('role_id')->where('member_id='.$_SESSION['member_id'])->find();
                $_SESSION['identity'] = $member_rolue_id['role_id'];
                if($_SESSION['identity']!=MEMBER_IDENTITY_TWO){
                    $contacts_name = $model->table('store_joinin')->field('company_name')->where('member_id='.$_SESSION['member_id'])->find();
                    if(!empty($contacts_name)){
                         $_SESSION['member_truename']=$contacts_name['company_name'];
                    }
                }
                //先查询是否有开店
//                $where_member['member_id'] = $member_info['member_id'];
//                $store_member = $model->table('store')->field('store_id')->where($where_member)->find();
//                if($store_member['store_id'] > 0){
//                    //这个会员是认证开店商家
//                    $_SESSION['identity']  = MEMBER_IDENTITY_FOUR;
//                }else{
//                    //如果是没有开店的则判断是否有认证过
//                    $store_joinin_where['joinin_state'] = STORE_JOIN_STATE_RZSUCCESS;
//                    $store_joinin_where['member_id'] = $member_info['member_id'];
//                    $store_joinin = $model->table('store_joinin')->field('member_id')->where($store_joinin_where)->find();
//                    //这个会员是认证未开店的会员
//                    if($store_joinin['member_id'] > 0){
//                        $_SESSION['identity'] =  MEMBER_IDENTITY_THREE;
//                    }else{
//                        //如果没有认证且没有认证的会员判断是否有city_id如果有则是采购员如果没有则是普通会员
//                        if($member_info['city_id']){
//                            $_SESSION['identity'] = MEMBER_IDENTITY_TWO;
//                        }else{
//                            $_SESSION['identity'] = MEMBER_IDENTITY_ONE;
//                        }
//                    }
//                }
        // 头衔COOKIE
        ...
        $_SESSION['expiretime'] = time() + LOGIN_SESSION_TIME;
        /*@Aletta end*/
    ...
    public function register($register_info) {
		...
		//cary_add 增加店铺会员
		$member_info['inviter_store']		= $register_info['inviter_store'];
	...
	public function addMember($param) {
		...
		try {
			...
			//cary_add 店铺会员注册
			$member_info['inviter_store']	        = $param['inviter_store'];
			//处理商家时长，默认数值为一年
			//$member_info['supply_end_time']	        = TIMESTAMP + (SUPPLY_TIME_LONG * 24 * 3600);
			/* @Aletta 2017.06.01*/
			//  手机注册登录绑定
			if ($param['supply_type']) {
				$member_info['supply_type'] = $param['supply_type'];
			}
			//  手机注册登录绑定
		...
		$member_info['member_wxopenid']		= $param['member_wxopenid'];
		    ...
		    if (!$insert) {
		        throw new Exception();
                    ...
                    if($insert_id > 0){
                        //添加会员注册 站内信提醒
                        $data_message_insert['to_member_id'] = $insert_id;
                        $data_message_insert['message_title'] = "恭喜您注册成为我们的会员！";
                        $data_message_insert['message_body'] = "恭喜您注册成为我们的会员！";
                        $data_message_insert['message_time'] = time()+1;
                        $data_message_insert['message_type'] = 1;
                        $data_message_insert['del_member_id'] = "";
                        $data_message_insert['read_member_id'] = "";
                        $data_message_insert['message_ismore'] = "";
                        $data_message_insert['from_member_name'] = "";
                        $data_message_insert['to_member_name'] = "";
                        $insert = $this->table('message')->insert($data_message_insert);
                        //处理商家时长，默认数值为一年
                        $sup_data = array(
                            'member_id'=>$insert_id,
                            "end_time"=>TIMESTAMP + (SUPPLY_TIME_LONG * 24 * 3600),
                            "add_time"=>TIMESTAMP,
                            "add_user"=>$insert_id
                        );
                        $this->table("supplier")->insert($sup_data);
                        //$member_info['supply_end_time']	        = TIMESTAMP + (SUPPLY_TIME_LONG * 24 * 3600);
                    }
            // 添加默认相册
            $insert = array();
	 ...
	 * @return	array 数组格式的返回结果
	 */
	public function addMemberForAdmin($param) {
		if(empty($param)) {
			return false;
		}
			...
			if ($param['supply_type']) {
				$member_info['supply_type'] = $param['supply_type'];
				$member_info['supply_code'] = $param['supply_code'];
				$member_info['role_id'] = $param['role_id'];
				$member_info['first_city_id'] = $param['first_city_id'];
			}
			//  手机注册登录绑定
			if ($param['member_mobile_bind']) {
				$member_info['member_mobile'] = $param['member_mobile'];
	 ...
	 * @param int $member_id
	 **/
	public function get_supply_level($member_id){
	    $member_level_info = $this->table('supplier')->field("level")->where("member_id = '".$member_id."'")->find();
	    if(!empty($member_level_info['level'])){
	        $level_data = array(
	            '1'=>'优选供应商',
	            '2'=>'合格供应商',
	            '3'=>'淘汰供应商',
	        );
	        return $level_data[$member_level_info['level']];
	    }
	}
	 ...
	 * @param int $member_id
	 **/
	public function get_supply_type($member_id){
	    $data_type = array();
	    $member_type_info = $this->table('supplier')->field("type_json")->where("member_id = '".$member_id."'")->find();
	    if(!empty($member_type_info['type_json'])){
	        $type_data = json_decode($member_type_info['type_json'],true);
	        if(!empty($type_data) && is_array($type_data)){
	            foreach ($type_data as $key=>$val){
	                $father_data = $this->table('supplier_type')->field('id,type_name')->where(array("level"=>1,"parent_id"=>0,'id'=>$key))->find();
	                $data_type[$key]['name'] = $father_data['type_name'];
	                foreach ($val as $v){
	                    $son_data = $this->table('supplier_type')->field('id,type_name')->where(array("level"=>2,"parent_id"=>$key,'id'=>$v))->find();
	                    $data_type[$key]['son'][] = array(
	                        "son_id"   => $son_data['id'],
	                        "son_name" => $son_data['type_name']
	                    );  
	                }
	            }
	        }
