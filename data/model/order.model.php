...
class orderModel extends Model {
     ...
     * @return unknown
     */
    public function getstore_order_detailed($condition = array(), $extend = array(), $fields = '*', $order = '',$group = '') {
        ...
        if (in_array('order_goods',$extend)) {
            ...
            $limit = null;
            $page = null;
            $order = 'goods_name desc';
            $order_goods_list = $this->getOrderGoodsList(array('order_id'=>$order_info['order_id']),$fields,$limit,$page,$order);
...
//        $list = $this->table('order')->field($field)->where($condition)->page($pagesize)->order($order)->limit($limit)->master($master)->select();
	 ...
	 * 获取订单日志
	 */
	public function getOrderLog($condition = array(), $fields = '*', $order = '', $limit = null) {
        return $this->table('order_log')->field($fields)->where($condition)->order($order)->limit($limit)->find();
	}
    ...
    public function getOrderOperateState($operate,$order_info){
        ...
        switch ($operate) {
            ...
            //买家取消订单
        	case 'buyer_cancel':
               if($_SESSION['identity'] == MEMBER_IDENTITY_ONE || $_SESSION['identity'] == MEMBER_IDENTITY_FIVE){
        	       $state = ($order_info['order_state'] == ORDER_STATE_NEW) ||
        	       ($order_info['payment_code'] == 'offline' && $order_info['order_state'] == ORDER_STATE_PAY);
                    }else{
                        if($order_info['order_state'] == ORDER_STATUS_SEND_ONE || $order_info['order_state'] == ORDER_STATUS_SUCCESS || $order_info['order_state'] == ORDER_DELIVER_GOODS){
                            $state = true;
                        }else{
                            $state = false;
                        }
    	   ...
    	   //商家取消订单
    	   case 'store_cancel':
    	       $state = ($order_info['order_state'] == ORDER_STATE_NEW || $order_info['order_state'] == ORDER_WAIT_CANCEL) ||  
           ...
           //商家接收订单
           case 'order_accept':
               log::record4inter("商订单状态:".$order_info['order_state']."`````````".ORDER_STATUS_SUCCESS, log::MOBILE_MESSAGE);
               $state = $order_info['order_state'] == ORDER_STATUS_SUCCESS ? true : false;
               break;
           //平台收款
	       ...
	       //买家投诉
	       case 'complain':
	           $state = in_array($order_info['order_state'],array(ORDER_STATE_PAY,ORDER_STATE_SEND,ORDER_STATUS_SEND_HET,ORDER_STATUS_CUS_RECEIVED)) ||
	    ...
	    //调整会员价
    	    ...
    	    case 'receive':
                if($_SESSION['identity'] == MEMBER_IDENTITY_TWO){
                    $state = !$order_info['lock_state'] && $order_info['order_state'] == ORDER_STATE_SEND||ORDER_STATUS_SEND_HET;
                }else{
        	...
        	case 'delete':
                ...
                //内部订单已检收回退
        	case 'check_back':
        	    $state = in_array($order_info['order_state'], array(ORDER_STATUS_CUS_RECEIVED,ORDER_STATE_SEND));
        	    break;
    ...
    public function getOrderAndOrderGoodsSalesRecordList($condition, $field="*", $page = 0, $order = 'rec_id desc') {
        $condition['order_state'] = array('in', array(ORDER_DELIVER_GOODS, ORDER_STATUS_SEND_HET, ORDER_STATUS_RECEIVED, ORDER_STATUS_CUS_RECEIVED, ORDER_STATUS_SUCCESS));
...
//print_r($state_type);
//print_r(str_replace($allow_state_array,    array(ORDER_STATE_NEW,ORDER_STATE_PAY,ORDER_STATE_SEND,ORDER_STATE_SUCCESS,ORDER_STATE_CANCEL), $state_type));
...
//            $condition['chain_code'] = array('gt',0);
        ...
        $order_list = $this->getOrderList($condition, 3, $fields, 'order_id desc','', $extend);//20
        //页面中显示那些操作
...
//                }
            ...
            $order_group_list[$key]['order_list']=$order_info;
        }
//print_r($order_list);
//        return $order_list;
...
//            $value['pay_sn'] = strval($key);
     ...
     * @param unknown $order_info
     */
     public function insert_oldorder($data){
         
        return $this->table('old_order')->insert($data);
     }
      /**
     * 新增插入数据 order表中数据
     * @param unknown $order_info
     */
     public function insert_newsorder($data){
