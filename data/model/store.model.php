...
class storeModel extends Model {
     ...
     * 查询需要手动推送认证的供应商
     * @date 2017-11
     * @param array $condition int $curpage int $showrow
     *
     */
    public function push_attestation($condition,$curpage) {
    	$field = "store_joinin.member_id,supplier.company_name,member.member_name,
    			 member.member_areainfo,store_joinin.contract_type,store_joinin.city_center";
    	$on = "store_joinin.member_id = member.member_id,member.member_id = supplier.member_id";
    	//默认显示未推送失败以及未推送的  ->order("store_joinin.member_id")
    	if (empty($condition) && !empty($curpage)) {
    		$condition = "store_joinin.contract_type != 1 and supplier.supplier_state = 2 and supplier.company_name != '' ";
    		return $this->field($field)->table('member,store_joinin,supplier')->join('left,left')->on($on)->where($condition)->page($curpage)->select();
    	}elseif (!empty($condition) && $curpage) {
    		if(($position = strpos($condition,'and')) !== false){
    			$leng = strlen('and');
    			$condition = substr_replace($condition,'',$position,$leng);
    		}  
    		return $this->field($field)->table('member,store_joinin,supplier')->join('left,left')->on($on)->where($condition)->page($curpage)->select();
    	}
    }
     ...
     *
     */
    public function store_result($condition){
    	if (!empty($condition)) {
    		$field = "id,city_name";
    		return $this->field($field)->table("city_centre")->where($condition)->find();
    	}
    }
    /**
     * 获取自营店铺的ID
     ...
     * @return array
	 */
    public function getStoreMemberIDList($storeid_array, $field = 'store_id,member_id,store_domain,store_city_id') {
...
/**
    ...
    private function getGoodsListBySales($store_array) {
    	$model = Model();
    	$field = 'goods_id,store_id,goods_name,goods_image,goods_price,goods_salenum,g_costprice';
    	foreach ($store_array as $value) {
