...
class dbModel{
	...
	public function getBackDir(){
		 ...
		 * 按照日期进行生成
		 */
		$dir_list = readDirList(BASE_ROOT_PATH.DS.'data/sql_back');
		$tmp = date('Ymd');
		$check_array = array();
	...
	public function writeBackFile(){
		...
		$step = $this->step;
		try {
			if (!is_dir(BASE_ROOT_PATH.DS.'data/sql_back'.DS.$_SESSION['db_backup']['back_file'])){
				if (!$this->mkdirs(BASE_ROOT_PATH.DS.'data/sql_back'.DS.$_SESSION['db_backup']['back_file'],0755)){
					$error = $lang['db_backup_mkdir_fail'];
					throw new Exception($error);
				}else {
					$fp = @fopen(BASE_ROOT_PATH.DS.'data/sql_back'.DS.$_SESSION['db_backup']['back_file'].DS.'index.html','w+');
					@fclose($fp);
				}
			}
			$file_name = BASE_ROOT_PATH.DS.'data/sql_back'.DS.$_SESSION['db_backup']['back_file'].DS.$_SESSION['db_backup']['back_file'].'_'.$step.'_'.$_SESSION['db_backup']['md5'].'.sql';
	...
	public function import($path,$step=1){
		$dir = BASE_ROOT_PATH.DS.'data/sql_back'.DS.$path;
		$file_list = array();
