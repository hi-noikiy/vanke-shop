...
class goodsModel extends Model{
	...
	$count = $this->getGoodsOnlineCount($condition,"distinct CONCAT(goods_commonid)");
        ...
        //获取当前商品的店铺ID 查询城市中心
        $city_where['store_id'] = array('in',$store_id);
        $model = Model();
        $cache_news = new CacheFile();
        $city = $cache_news->get('city_name');
        if(!empty($goods_list) && is_array($goods_list) && !empty($city) && is_array($city) ){
            foreach($goods_list as $key=>$rows){
                foreach($city as $city_name){
                    if($rows['store_id'] == $city_name['id']){
                        $goods_list[$key]['store_city_name'] = $city_name['city_name'];
                    }
                }
            }
        }
    ...
    //商品
 public function getGoodsListByColorDistinct2($condition, $field = '*', $order = 'goods_id asc', $page = 0) {
        $condition.=' AND ( '.'goods_state = '.self::STATE1.' or goods_state = '.self::STATE2.') ';
        $condition.=' AND ( '.'goods_verify = '.self::VERIFY1.' ) AND del = 1';
        $model = Model();
        $condition = $this->_getRecursiveClass($condition);
	$field = "CONCAT(goods_commonid) as nc_distinct ," . $field;
        //统计商品总数
        $count=$model->table('goods')->where($condition)->group('')->count1("distinct CONCAT(goods_commonid)");  
	//$count = $this->getGoodsOnlineCount($condition,"distinct CONCAT(goods_commonid)");
        $goods_list = array();
        if ($count != 0) {
            //获取商品list
            $goods_list=$model->table('goods')->field($field)->where($condition)->group( 'nc_distinct')->order($order)->limit(0)->page($page, $count)->lock(false)->select(); 
          //$goods_list = $this->getGoodsOnlineList($condition, $field, $page, $order, 0, 'nc_distinct', false, $count);
            if(!empty($goods_list) && is_array($goods_list)){
                ...
                //获取当前商品的店铺ID 查询城市中心
                $city_where['store_id'] = array('in',implode(',',$store_id));
                $cache_news = new CacheFile();
                $city = $cache_news->get('city_name');
                if(is_array($goods_list) && is_array($city)){
                    foreach($goods_list as $key=>$rows){
                        foreach($city as $city_name){
                            if($rows['store_id'] == $city_name['id']){
                                $goods_list[$key]['store_city_name'] = $city_name['city_name'];
    ...
    public function getGoodsCommonList($condition, $field = '*', $page = 10, $order = 'goods_commonid desc') {
        $condition = $this->_getRecursiveClass($condition);
        return $this->table('goods_common,store')->field($field)->join('left join')->on('store.store_id = goods_common.store_id')->where($condition)->order($order)->page($page)->select();
    }
     ...
     * @return array
     */
    public function getGoodsCommonList1($condition, $field = '*', $page = 10, $order = 'goods_commonid desc') {
        $condition = $this->_getRecursiveClass($condition);
    ...
    public function getGoodsCommonOnlineList($condition, $field = '*', $page = 10, $order = "goods_commonid desc") {
        $condition['goods_state']   = self::STATE1;
        $condition['goods_verify']  = self::VERIFY1;
        return $this->getGoodsCommonList1($condition, $field, $page, $order);
    }
    ...
    public function getGoodsCommonListForPromotion($condition, $field = '*', $page = 10, $type) {
        ...
        return $this->getGoodsCommonList1($condition, $field, $page);
    }
     ...
     * @return array
     */
    public function getGoodsCommonOffVerifyList($condition, $field = '*', $page = 10, $order = "goods_commonid desc") {
        if (!isset($condition['goods_state'])) {
            $condition['goods_state']  = array('eq', self::STATE2);
        }
        return $this->getGoodsCommonList1($condition, $field, $page, $order);
    }

     /**
     * 等待审核或审核失败的商品列表 总后台中心使用
     *
     * @param array $condition 条件
     * @param array $field 字段
     * @param string $page 分页
     ...
     * @return array
     */
    public function getGoodsCommonWaitVerifyList2($condition, $field = '*', $page = 10, $order = "goods_commonid desc") {
        if (!isset($condition['goods_verify'])) {
            $condition['goods_verify']  = array('neq', self::VERIFY1);
        }
    ...
    public function getGoodsCommonWaitVerifyList3($condition, $field = '*', $page = 10, $order = "goods_commonid desc") {
        return $this->getGoodsCommonList($condition, $field, $page, $order);
    }
    ...
    public function getGoodsCommendList($store_id, $limit = 5) {
            $goods_commend_list = $this->getGoodsOnlineList(array('store_id' => $store_id, 'goods_commend' => 1), 'goods_id,goods_name,goods_jingle,goods_image,store_id,goods_promotion_price,g_costprice,goods_price  ', 0, 'rand()', $limit, 'goods_commonid');
            if (!empty($goods_id_list)) {
                $tmp = array();
                foreach ($goods_id_list as $v) {
                    $tmp[] = $v['goods_id'];
                }
                $goods_commend_list = $this->getGoodsOnlineList(array('goods_id' => array('in',$tmp)), 'goods_id,goods_name,goods_jingle,goods_image,store_id,goods_promotion_price,g_costprice,goods_price', 0, 'rand()', $limit);
            }
    ...
    public function editGoodsCommon($update, $condition) {
        $common_list = $this->getGoodsCommonList1($condition, 'goods_commonid', 0);
    ...
    public function editProduces($condition, $update1, $update2 = array()) {
        ...
        $goods_array = $this->getGoodsCommonList1($condition, 'goods_commonid', 0);
    ...
    public function editProducesVerifyFail($condition, $update1, $update2 = array()) {
        ...
        if ($result) {
            $commonlist = $this->getGoodsCommonList1($condition, 'goods_commonid,store_id,goods_verifyremark', 0);
    ...
    public function editProducesOffline($condition){
        $update = array('goods_state' => self::STATE2);
     ...
     * @return boolean
     */
    public function editProducesOffline_s($condition){
     ...
     * @author Aletta
     */
    public function editProducesOnlineVerification($condition){
        //查询是否存在符合记录的数据
        $condition['goods_state'] = self::STATE0;
        $goods_com_data = $this->table('goods_common')->where($condition)->find();
        if(!empty($goods_com_data)){
            $res = $this->table('goods_common')->where($condition)->update(array('goods_state'=>self::VERIFY1,'goods_verify'=>self::VERIFY10));
            if($res){
                unset($condition['goods_state']);
                $this->table('goods')->where($condition)->update(array('goods_state'=>self::VERIFY1,'goods_verify'=>self::VERIFY10));
            }
    ...
    public function editProducesLockUp($update, $condition) {
        ...
        if ($return) {
            // 商品违规下架发送店铺消息
            $common_list = $this->getGoodsCommonList1($condition, 'goods_commonid,store_id,goods_stateremark', 0);
            foreach ($common_list as $val) {
                $param = array();
    ...
    public function getGoodsOnlineInfoAndPromotionById($goods_id) {
        ...
        if (empty($goods_info) || ( ($goods_info['goods_state'] != self::STATE1 && $goods_info['goods_state'] != self::STATE2) || $goods_info['goods_verify'] != self::VERIFY1)) {
            return array();
        }
        return $goods_info;
    }
    ...
    public function delGoods($condition) {
        ...
        return $this->table('goods')->where($condition)->update(array('del'=>2));
    }
    ...
    public function delGoodsAll($condition) {
        ...
        // 删除商品公共表数据
        $this->table('goods_common')->where(array('goods_commonid' => array('in', $commonid_array)))->update(array('del'=>2));
        // 删除商品图片表数据
    ...
    public function delGoodsNoLock($condition) {
        $condition['goods_lock'] = 0;
        $common_array = $this->getGoodsCommonList1($condition, 'goods_commonid', 0);
