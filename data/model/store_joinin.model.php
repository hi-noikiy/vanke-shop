...
class store_joininModel extends Model{
	...
	public function getList($condition,$page='',$order='',$field='*'){
        ...
        $result = $this->table('store_joinin,supplier')->field($field)->join('left')->on($on)->where($condition)->page($page)->order($order)->select();
        return $result;
	}
    ...
    public function getOne($condition){
        $on = "store_joinin.member_id = supplier.member_id";
        $result = $this->table('store_joinin,supplier')->join('left')->on($on)->where($condition)->find();
        return $result;
    }
