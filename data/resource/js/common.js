...
//弹出框登录
function login_dialog_4iframe(url){
    CUR_DIALOG = ajax_form('login','登录',SITEURL+'/index.php?act=login&inajax=1&ref_url_iframe='+url,360,1);
}
/* 显示Ajax表单 */
    ...
    $.getJSON(SITEURL+'/index.php?act=cart&op=del&cart_id='+cart_id+'&goods_id='+goods_id+'&callback=?', function(result){
        if(result.state){
            ...
            if(result.quantity == 0){
				...
				$('.cart-list').html('<li><dl style="width: 100%; margin-left: 0;"><dd style="text-align: center; ">暂无商品</dd></dl></li>');
...
function resetCarListHeight() {
    var conHeight = $('#content-cart').outerHeight();
    var topHeight = $('#content-cart>.top').outerHeight();
    var bomHeight = $('#rtoolbar_cartlist>.btn-box').outerHeight();
    var setHeight = conHeight - topHeight - bomHeight;
    $('.cart-list').height(setHeight);
}
    ...
    $.getJSON(url, {'goods_id':goods_id, 'quantity':quantity}, function(data) {
        if (data != null) {
            if (data.state) {
                ...
                $("#rtoolbar_cartlist").load(SHOP_SITE_URL + '/index.php?act=cart&op=ajax_load&type=html', resetCarListHeight);
            } else {
                alert(data.msg);
            }
        }
...
/* 加入购物车 */
function addCartBatch(goodIdArray,callbackfunc) {
    var url = 'index.php?act=cart&op=addCartBatch';
    $.getJSON(url, {'goodIdArray':goodIdArray}, function(data) {
        if (data != null) {
            if (data.state) {
                if(callbackfunc){
                    eval(callbackfunc + "(data)");
...
function getCookieForUTF8(c_name){
...
function setCookieByMilliAndEncodeURI(c_name,value,milliS,path){
    var exdate=new Date();
    exdate.setTime(exdate.getTime()+milliS);
    //var v = (value)?value:'/';
    document.cookie=c_name+ "=" +encodeURIComponent(value)+
        ((milliS==null) ? "" : "; expires="+exdate.toGMTString())+";path="+((path==null||path==='undefined') ? "/":path);
}
