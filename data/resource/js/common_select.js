...
function gcategoryChange()
{
    ...
    $(this).parent().find(".mls_id,.mls_id2").val(id);
    $(this).parent().find(".mls_name,.mls_name2").val(name);
    $(this).parent().find(".mls_names,mls_names2").val(names.join("\t"));
...
function editproduct_code(){
    $(".class-select").bind("change",function(){
            var classid = $('.mls_id').val();
            $.post(
                     ADMINURL+ '/index.php?act=codemanages&op=getclasscode',
                    {
                        'classid':classid
                    },
                    function(data){
