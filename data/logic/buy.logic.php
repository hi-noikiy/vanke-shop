...
class buyLogic {
    ...
    public function getGoodsList($cart_id, $member_id, $store_id) {
        ...
        if(empty($goods_info)) {
            return callback(false, '商品已下架,或者超出最大购买限制');//商品已下架或不存在
        }
    ...
    private function _createOrderStep4() {
        ...
        $inviter_store = $this->_member_info['inviter_store']; //cary_add 添加店铺注册推广 ID
...
//print_r($store_freight_total[$store_id]);exit;
            ...
            $order['inviter_store'] = $buyer_info['inviter_store']; // cary_add 邀请店铺ID
            $order['code_combination_id']   =  $_POST['obj_id'];

            $order['add_time'] = TIMESTAMP;
            $order['payment_code'] = $store_pay_type_list[$store_id];
            if($_SESSION['city_id']){
                $order['order_lei'] = 2;
                $order['order_state'] = ORDER_STATUS_SEND_ONE;
            }else{
                $order['order_lei'] = 1;
...
//print_r($store_freight_total);exit;
            $order_id = $model_order->addOrder($order);
            
//            $send_apiorder = $this->_api_sendorderOp($order);
//            if($send_apiorder == "success"){
//                //将订单推向合同系统，返回成功之后将订单状态改成审核中60
//                $data_updata['order_state'] = ORDER_STATUS_SEND_TWO;
//                $updata_sen_status = $model_order->where('order_id='.$order_id)->update($data_updata);
//            }
            if (!$order_id) {
            ...
            foreach ($goods_list as $goods_info) {
                ...
                if (!intval($goods_info['bl_id'])) {
                    ...
                    //cary_replace  加入商品的佣金比率
                    //$order_goods[$i]['commis_rate'] =floatval($store_gc_id_commis_rate_list[$store_id][$goods_info['gc_id']]);
                    // to
                    if($goods_info['goods_commis_end'] == 0 || $goods_info['goods_commis_end'] > TIMESTAMP){
                        $order_goods[$i]['commis_rate'] =floatval($goods_info['goods_commis_rate']);
                    }else{
                        $order_goods[$i]['commis_rate'] =floatval($store_gc_id_commis_rate_list[$store_id][$goods_info['gc_id']]);
                    }
                    // end
    ...
    //TODO
    private function _api_sendorderOp($data){
