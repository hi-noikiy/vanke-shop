    ...
    public function getStoreFreightList($buy_list = array(), $free_freight_sid_list) {
        ...
        foreach ($buy_list as $key => $goods_info) {
            ...
            if (!intval($goods_info['transport_id']) &&  !in_array($goods_info['store_id'],$free_freight_sid_list)) {
                $freight_list[$goods_info['store_id']] = $goods_info['goods_freight'];//出错地点
                unset($buy_list[$key]);
            }
        }
        //计算优惠套装商品运费
...
//         if (empty($pd_payment_info)) return;
  ...
  public function getFreeFreightActiveLists($store_id_array) {
        if (empty($store_id_array) || !is_array($store_id_array)) return array();
        ...
        foreach ($store_list as $store_info) {
            ...
            if ($limit_price > 0) {
//                $store_free_freight_active[$store_info['store_id']] = sprintf('满%s免运费',$limit_price);
                 $store_free_freight_active[$store_info['store_id']] = $limit_price;
            }
        }
        return $store_free_freight_active;
    }
    ...
    public function createInvoiceData($invoice_info){
        ...
        if ($invoice_info['inv_state'] == 1) {
            $inv['类型'] = '普通发票 '."<br>";            
            $inv['抬头'] = $invoice_info['inv_title_select'] == 'person' ? '个人' ."<br>": $invoice_info['inv_title']."<br>";           
            $retus = $invoice_info['inv_title'] == '个人' ?  true : false;
            if(!$retus){
              $inv['纳税人识别号'] =$invoice_info['inv_code']."<br>";  
            }           
            $inv['内容'] = $invoice_info['inv_content']."<br>";
            $inv['收票人姓名'] = $invoice_info['inv_rec_name']."<br>";
            $inv['收票人手机号'] = $invoice_info['inv_rec_mobphone']."<br>";
            $inv['收票人省份'] = $invoice_info['inv_rec_province']."<br>";
            $inv['收票地址'] = $invoice_info['inv_goto_addr']."<br>";
        } elseif (!empty($invoice_info)) {
            $inv['单位名称'] = $invoice_info['inv_company']."<br>";
            $inv['纳税人识别号'] = $invoice_info['inv_code']."<br>";
            $inv['注册地址'] = $invoice_info['inv_reg_addr']."<br>";
            $inv['注册电话'] = $invoice_info['inv_reg_phone']."<br>";
            $inv['开户银行'] = $invoice_info['inv_reg_bname']."<br>";
            $inv['银行账户'] = $invoice_info['inv_reg_baccount']."<br>";
            $inv['收票人姓名'] = $invoice_info['inv_rec_name']."<br>";
            $inv['收票人手机号'] = $invoice_info['inv_rec_mobphone']."<br>";
            $inv['收票人省份'] = $invoice_info['inv_rec_province']."<br>";
            $inv['收票地址'] = $invoice_info['inv_goto_addr']."<br>";
    ...
    private function _getGoodsOnlineInfo($goods_id,$quantity) {
        ...
        //$goods_info['store_id'];
        $store_id=  Model();
        $freight=$store_id->table('store')->where(array('store_id'=>$goods_info['store_id']))->field('freight')->find();
        if(empty($goods_info)){
            return null;
        }
        $new_array = array();
...
//        $new_array['goods_price'] = $goods_info['goods_price'];
        if($_SESSION['identity'] == MEMBER_IDENTITY_TWO){//这是采购员
            ...
            $new_array['goods_price'] = $goods_info['goods_third_price'];
        }else{
            $new_array['goods_price'] = $goods_info['goods_toc_price'];
        }
        ...
        $new_array['goods_freight'] = $freight['freight'];//$goods_info['goods_freight'];
        ...
        //cary_add :  添加商品级佣金比率和截止时间, 从goods_common 表中获取相关信息
        $goods_common = Model('goods')->getGoodeCommonInfoByID($goods_info['goods_commonid'],'goods_commis_rate,goods_commis_end');
        $new_array['goods_commis_rate'] = $goods_common['goods_commis_rate'];
        $new_array['goods_commis_end'] = $goods_common['goods_commis_end'];
        //add_end
    ...
    private function _getOnlineCartList($cart_list) {
        ...
        foreach ((array)$cart_list as $key => $cart_info) {
             $freight=$store_id->table('store')->where(array('store_id'=>$cart_info['store_id']))->field('freight')->find();
            ...
            if (in_array($cart_info['goods_id'],array_keys($goods_online_array))) {
                ...
                if($_SESSION['identity'] == MEMBER_IDENTITY_TWO){//这是采购员
                    ...
                    $cart_list[$key]['goods_price'] = $goods_online_info['goods_third_price'];
                }else{
                    $cart_list[$key]['goods_price'] = $goods_online_info['goods_third_price'];
                }
//                $cart_list[$key]['goods_price'] = $goods_online_info['goods_price'];
                $cart_list[$key]['transport_id'] = $goods_online_info['transport_id'];
                $cart_list[$key]['goods_freight'] = $freight['freight'];//$goods_online_info['goods_freight'];
                ...
                $cart_list[$key]['good_min'] = $goods_online_info['min_num'];
                $cart_list[$key]['good_max'] = $goods_online_info['max_num'];
