...
class orderLogic {
    ...
    public function changeOrderStateCancel($order_info, $role, $user = '', $msg = '', $if_update_account = true, $if_quque = true) {
        try {
            ...
            /** --------------------卖家操作k2审核过取消订单-------------------------------- **/
            if($order_info['order_state'] == ORDER_WAIT_CANCEL && $role == 'seller'){
                //获取订单取消的原因
                $order_log_info = $model_order->getOrderLog(array("order_id"=>$order_id,"log_orderstate"=>1),"","log_time desc",1);
                //发起订单取消数据推送
                $send_data = array(
                    "order_sn"      => $order_info['order_sn'],
                    "order_state"   => "90",
                    "cancel_reason" => $order_log_info['log_msg'],
                );
                $send_json = json_encode($send_data);
                $url=YMA_WEBSERVICE_CANCEL_ORDER;
                $return_json = WebServiceUtil::getDataByCurl($url, $send_json, 0);
                $return = json_decode($return_json,true);
                if($return['resultCode'] != '0'){
                    return callback(false,$return['resultMsg']);
                }
            }
            ...
            //更新订单信息
            /*无法确认is_class来源，现将其去除，后续如有确认，在进行还原  
             * ************   Start   原代码    ************
            if($order_info['is_class'] == 1){
                $update_order = array('order_state' => ORDER_STATE_CANCEL, 'pd_amount' => 0);
            }else{
                $update_order = array('pd_amount' => 0);
            } 
            ...
            ******************** end  ****************/
            ...
            /* ************* 修改位置 *******************/
            $update = $model_order->editOrder(array('order_state' => ORDER_STATE_CANCEL, 'pd_amount' => 0),array('order_id'=>$order_id));
            /* ***************************************/
     ...
     * @return array
     */
    public function changeExamineOrderStateCancel($order_info, $role, $user = '', $msg = ''){
        try {
        $model_order = Model('order');
        //修改订单状态
        $update = $model_order->editOrder(array('order_state' => ORDER_WAIT_CANCEL),array('order_id'=>$order_info['order_id']));    
        if (!$update) {
            throw new Exception('保存失败');
        }
        ...
        $data['log_role'] = $role;//操作角色
        $data['log_msg'] = '取消了订单';
        $data['log_user'] = $user;//操作人
        $data['original_state'] = $order_info['order_state'];//原先的订单状态，如果供应商不同意从这边取值还原
        if ($msg) {
            $data['log_msg'] .= ' ( '.$msg.' )';
        }
        $data['log_orderstate'] = ORDER_WAIT_CANCEL;
        $model_order->addOrderLog($data);//将取消订单信息添加到这边
     ...
     * @return array
     */
    public function changeOrderAccept($order_info, $role, $user = '', $msg = ''){
        try {
            $model_order = Model('order');
            $update = $model_order->editOrder(array('order_state' => ORDER_DELIVER_GOODS),array('order_id'=>$order_info['order_id']));
            if (!$update) {
                throw new Exception('保存失败');
            }
            ...
            $data['order_id'] = $order_info['order_id'];
            $data['log_role'] = $role;
            $data['log_msg'] = '受理了买家订单';
            $data['log_user'] = $user;
            $data['log_orderstate'] = ORDER_DELIVER_GOODS;
            $model_order->addOrderLog($data);
    ...
    public function changeOrderSend($order_info, $role, $user = '', $post = array()) {
		...
		try {
            ...
            if($order_info['order_lei']=="1"){
                $data['order_state'] = ORDER_STATE_SEND;
            }elseif($order_info['order_lei']=="2"){
