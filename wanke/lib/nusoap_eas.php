...
class nusoap_base {
	...
	*/
	var 	 $namespaces = array(
		...
		'bat' => 'http://www.vanke.com/eas/batchAddSupplierWYService/',
		'v1' => 'http://www.vanke.com/eas/batchAddSupplierWY/v1',
		'osb' => 'http://www.vanke.com/osb'
		);
    ...
    */
	...
	// serialize envelope
        $aaa = '<?xml version="1.0" encoding="'.$this->soap_defencoding .'"?'.">".
...
* @access   public
*/
class nusoap_client_eas extends nusoap_base  {
	...
	* @access   public
	*/
	function nusoap_client_eas($endpoint,$wsdl = false,$proxyhost = false,$proxyport = false,$proxyusername = false, $proxypassword = false, $timeout = 0, $response_timeout = 30, $portName = ''){
		parent::nusoap_base();
	...
	*/
	function call($operation,$params=array(),$namespace='http://tempuri.org',$soapAction='',$headers=false,$rpcParams=null,$style='rpc',$use='encoded'){
		...
		// serialize envelope supplierInfo
           			$usedNamespaces = array();     
            if ($use == 'encoded') {
		$encodingStyle = 'http://schemas.xmlsoap.org/soap/encoding/';
	    } 
                //将参数重新赋值 拼接正确的报文 仅对应万科使用
                 $payload = "<bat:requestDataRQ>";
                 $supplierInfo = $params['requestDataRQ']['supplierInfo'];
//                 var_dump($supplierInfo);exit;
                    $payload.="<supplierInfo>"
                            . "<v1:Number>".$supplierInfo['Number']."</v1:Number>"
                            . "<v1:Name>".$supplierInfo['Name']."</v1:Name>"
                            . "<v1:standardNum>".$supplierInfo['standardNum']."</v1:standardNum>"
                            . "<v1:groupNumber>".$supplierInfo['groupNumber']."</v1:groupNumber>"
                            . "<v1:orgNumber>".$supplierInfo['orgNumber']."</v1:orgNumber>"
                            . "</supplierInfo>";
                
                if($params['requestDataRQ']['requestPubProfile']!=""){
                    $requestInfo = $params['requestDataRQ']['requestPubProfile']['requestInfo'];
                    $payload.="<requestPubProfile>"
                            . "<osb:requestInfo>"
                            . "<osb:requestID>".$requestInfo['requestID']."</osb:requestID>"
                            . "<osb:correlationID>".$requestInfo['correlationID']."</osb:correlationID>"
                            . "<osb:version>".$requestInfo['version']."</osb:version>"
                            ."</osb:requestInfo>"
                             ."</requestPubProfile>";
                }
                if($params['requestDataRQ']['batchType']!=""){
                    $batchInfo = $params['requestDataRQ']['batchType']['batchInfo'];
                    $payload.="<batchType>"
                            . "<osb:batchInfo>"
                            . "<osb:dataName>".$batchInfo['dataName']."</osb:dataName>"
                            . "<osb:dataCount>".$batchInfo['dataCount']."</osb:dataCount>"
                            . "</osb:batchInfo>"
                            . "</batchType>";
                }
                $Systems = $params['requestDataRQ']['Systems'];
                $payload.="<Systems>"
                        . "<osb:system sysName=\"EAS\">".$Systems['system']."</osb:system>"
                        . "<osb:source sysName=\"WYCG\">".$Systems['source']."</osb:source>"
                        . "</Systems>";
...
if (!extension_loaded('soap')) {
	 ...
	 */
	class soapclient_eas extends nusoap_client_eas {
	}
}
?>
