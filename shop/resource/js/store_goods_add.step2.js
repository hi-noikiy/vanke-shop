...
// 计算价格
function computemarkPrice(){
    // 计算最低价格
    var _price = 0;var _price_sign = false;
    $('input[data_type="marketprice"]').each(function(){
        if($(this).val() != '' && $(this)){
            if(!_price_sign){
                _price = parseFloat($(this).val());
                _price_sign = true;
    ...
    $('input[name="g_marketprice"]').val(number_format(_price, 2));

    discountCalculator();       // 计算折扣
}

// 计算价格
function computegcosprice(){
    // 计算最低价格
    var _price = 0;var _price_sign = false;
    $('input[data_type="g_costprice"]').each(function(){
        if($(this).val() != '' && $(this)){
            if(!_price_sign){
                _price = parseFloat($(this).val());
                _price_sign = true;
...
// 计算价格
function computetocprice(){
    // 计算最低价格
    var _price = 0;var _price_sign = false;
    $('input[data_type="goods_toc_price"]').each(function(){
        if($(this).val() != '' && $(this)){
            if(!_price_sign){
                _price = parseFloat($(this).val());
                _price_sign = true;
    ...
    $('input[name="goods_toc_price"]').val(number_format(_price, 2));

}

// 计算价格
function computethrprice(){
    // 计算最低价格
    var _price = 0;var _price_sign = false;
    $('input[data_type="goods_third_price"]').each(function(){
        if($(this).val() != '' && $(this)){
            if(!_price_sign){
                _price = parseFloat($(this).val());
                _price_sign = true;
    ...
    $('input[name="goods_third_price"]').val(number_format(_price, 2));
