(function($) {
	...
	$.fn.jfocus = function(settings) {//首页焦点广告图切换
		...
		return this.each(function(){
			...
			$this.hover(function() {
					$this.find(".arrow").css("opacity", 0.4);
...
$(function(){
	...
	/*
	 ...
	 $(".tabs-slider").eq(index).removeClass("tabs-hide").siblings(".tabs-slider").addClass("tabs-hide");
	 },function(){});
	 */
	$(".tabs-slider").addClass("tabs-hide");
	$(".tabs-slider").eq(0).removeClass("tabs-hide")
	...
	//首页Tab标签卡滑门切换
	$(".recommend-classes > ul > li > a").bind('mouseover', (function(e) {
		if (e.target == this) {
			var tabs = $(this).parent().parent().children("li");
			var index = $.inArray(this, $(this).parent().parent().find("a"));
			var layout = $(this).parent().parent().parent().parent().parent().find(".middle-layout");
			var panels = layout.children(".tabs-panel");
			if (panels.eq(index)[0]) {
				tabs.removeClass("tabs-selected1").eq(index).addClass("tabs-selected1");
				panels.addClass("tabs-hide").eq(index).removeClass("tabs-hide");
			}
		}
	}));
	...
	$(".jfocus-trigeminy").fullScreen({time:3000,css:"hide"});
	...
	$('.nav_Sidebar a').each(function(index){
		$(this).click(function(){
			if( index < $('.home-standard-layout').size()){
				$('html,body').animate({scrollTop: $('.home-standard-layout').eq(index).offset().top - 20 + 'px'}, 500);
			}else{
				window.open( SHOP_SITE_URL+'/index.php?act=search&op=index&cate_id=1','_blank');
			}
		}).mouseover(function(){
