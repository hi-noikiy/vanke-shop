...
class store_goods_offlineControl extends BaseSellerControl {
    ...
    public function goods_storageOp() {
        ...
        switch ($_GET['type']) {
            ...
            //下架审核
            case 'off_verify':
                $this->profile_menu('goods_offvfy');
                $goods_list = $model_goods->getGoodsCommonOffVerifyList($where);
                break;
            // 仓库中的商品
            default:
        ...
        switch ($_GET['type']) {
            ...
            case 'wait_verify':
                ...
                break;
            case 'off_verify':
                Tpl::showpage('store_goods_list.off.verify');
                break;
            // 仓库中的商品
            default:
    ...
    public function goods_showOp() {
        ...
        $return = Model('goods')->editProducesOnlineVerification(array('goods_commonid' => array('in', $commonid_array), 'store_id' => $_SESSION['store_id']));
        ...
        if ($return) {
            ...
            showDialog('商品上架成功，请等待管理员的审核', 'reload', 'succ');
        } else {
    ...
    private function profile_menu($menu_key = '') {
        $menu_array = array(
            ...
            array(
                ...
                'menu_url' => urlShop('store_goods_offline', 'index', array('type' => 'lock_up'))
            ),
            array(
                'menu_key' => 'goods_verify',
                'menu_name' => L('nc_member_path_goods_verify'),
                'menu_url' => urlShop('store_goods_offline', 'index', array('type' => 'wait_verify'))
            ),
            array(
                'menu_key' => 'goods_offvfy',
                'menu_name' => '商品下架审核中',
                'menu_url' => urlShop('store_goods_offline', 'index', array('type' => 'off_verify'))
            ),
        );
