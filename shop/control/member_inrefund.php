...
class member_inrefundControl extends BaseMemberControl {
	...
	public function add_refundOp(){
		...
		if ($order_id < 1 || $goods_id < 1) {//参数验证
                    ...
                    showDialog(Language::get('wrong_argument'),'index.php?act=member_inorder&op=inside_order','error');
		}
		$condition = array();
		...
		if ($refund['refund_id'] > 0 || $order['order_state'] != 33) {//检查订单状态,防止页面刷新不及时造成数据错误	
                    showDialog(Language::get('wrong_argument'),'index.php?act=member_inorder&op=inside_order','error');
		}
		if (chksubmit() && $goods_id > 0){
			...
			if ($refund_array['refund_type'] != '2') {
			    ...
			    $show_url = 'index.php?act=member_inrefund&op=inside_order';
			}
			...
			$refund_array['inviter_store'] = $order['inviter_store']; //cary_add 店铺邀请ID
		...
		Tpl::showpage('member_inrefund_add');
	}
	/**
	 * 添加全部退款即取消订单
	 *
	...
	public function add_refund_allOp(){
		...
		if (chksubmit()) {
			...
			if ($state) {
			    $model_refund->editOrderLock($order_id);
				showDialog(Language::get('nc_common_save_succ'),'index.php?act=member_inrefund&op=index','succ');
			} else {
				showDialog(Language::get('nc_common_save_fail'),'reload','error');
			}
		}
	...
	private function profile_menu($menu_type,$menu_key='') {
		...
		switch ($menu_type) {
			case 'member_order':
				$menu_array = array(
				array('menu_key'=>'buyer_refund','menu_name'=>Language::get('nc_member_path_buyer_refund'),	'menu_url'=>'index.php?act=member_inrefund'),
				array('menu_key'=>'buyer_return','menu_name'=>Language::get('nc_member_path_buyer_return'),	'menu_url'=>'index.php?act=member_inreturn'),
