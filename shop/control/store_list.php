...
class store_listControl extends BaseHomeControl {
	...
	public function indexOp(){
        ...
        foreach($store_list as $key=>$s_rows){
            unset($city_name);
            $store_city_id = explode(',',$s_rows['store_city_id']);
            if($city){
                foreach($store_city_id as $c_rows){
                    foreach($city as $c_id){
                        if($c_rows == $c_id['id']){
                            $city_name .= $c_id['city_name'].',';
                        }
                    }
                }
            }
            $store_list[$key]['store_city_name'] = substr($city_name, 0,-1);
        }
		...
		//SEO
		Model('seo')->type(array('商城 - 店铺','商城 - 店铺','商城 - 店铺'))->show();
