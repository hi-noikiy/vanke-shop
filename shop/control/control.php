...
class Control{
	...
	$model_channel = Model('web_channel');
        ...
        Tpl::output('show_goods_class',$goods_class);//商品分类
          //暂时去除游客角色。必须登录后才能使用系统
        if($_SESSION['is_login']!="1"&&$_GET['act']!="login"){
            if($_GET['act']!="connect_sms"){
                if($_SESSION['is_admin_login']!="1"){
		            if($_GET['act']!="connect_wk"){
			            @header('Location: index.php?act=login&op=index');exit;
                    }
    ...
    protected function checkLogin(){
        if($_SESSION['expiretime'] < TIMESTAMP) {
    ...
    protected function article() {
        ...
        if (C('cache_open')) {
            ...
            if(!file_exists(BASE_DATA_PATH . DS . "cache" . DS . "index")){
                $file_obj->createDir(BASE_DATA_PATH . DS . "cache" . DS . "index");
            }
            if(!file_exists(BASE_DATA_PATH.'/cache/index/article.php')){
                $file_obj->createFile(BASE_DATA_PATH.'/cache/index/article.php');
            }
    ...
    protected function getSendUrl(){
        if($this->is_https()){
            return "http://120.77.89.93:8080";//生产地址
        }else{
            return "http://120.77.38.59:8080";//测试地址
        }
    }
}
...
class BaseMemberControl extends Control {
    ...
    private function _getMenuList() {
        $online_inquiry= BASE_SITE_URL.'/index.php?act=member_unline&op=tenderInquiry&mtype=inquiry&up=mc';
        $menu_list = array(
                'trade' => array('name' => '交易管理', 'child' => array(
                        //'member_inorder'      => array('name' => '订单查询', 'url'=>urlShop('member_inorder', 'inside_order')),
...
//                        'member_vr_order'   => array('name' => '虚拟兑码订单', 'url'=>urlShop('member_vr_order','index')),
                ...
                'info' => array('name' => '资料管理', 'child' => array(
                       ...
                       // ,'impc_Interface'=> array('name' => '接口测试', 'url'=>urlShop('impc_Interface', 'index'))
                ))
        );
        //分角色显示菜单
        if($_SESSION['identity'] == MEMBER_IDENTITY_THREE  ){//02认证供应商，未开店
            $menu_list['trade']['child']=array('member_unline'  => array('name' => '线下订单', 'url'=>urlShop('member_unline', 'index')));
            unset($menu_list['serv'] );
            unset($menu_list['info'] );
            $menu_list['tender_inquiry'] = array('name' => '商机管理', 'child' => array(
                'online_inquiry'   => array('name' => '在线询价', 'url'=>$online_inquiry),
            ));
            $menu_list['info'] = array('name' => '资料管理', 'child' => array(
                'member_information'=> array('name' => '账户信息', 'url'=>urlShop('member_information', 'member')),
            ));
        }else if($_SESSION['identity'] == MEMBER_IDENTITY_TWO){
            $menu_list['trade']['child']['member_inorder']  = array('name' => '订单查询', 'url'=>urlShop('member_inorder', 'inside_order'));
            $menu_list['trade']['child']['get_goods']  = array('name' => '快速下单', 'url'=>urlShop('member_information', 'get_goods'));
            $menu_list['serv']['child']['rule_list']  = array('name' => '采购制度', 'url'=>urlShop('article', 'rule_list'));
            //$menu_list['trade']['child']['member_unline']=array('name' => '线下订单', 'url'=>urlShop('member_unline', 'index'));
        }else if($_SESSION['identity'] == MEMBER_IDENTITY_FOUR){
            $menu_list['trade']['child']=array('member_unline'  => array('name' => '线下订单', 'url'=>urlShop('member_unline', 'index')));
            //unset($menu_list['trade'] );
            unset($menu_list['serv'] );
            unset($menu_list['info'] );
            $menu_list['tender_inquiry'] = array('name' => '商机管理', 'child' => array(
                'online_inquiry'   => array('name' => '在线询价', 'url'=>$online_inquiry),
            ...
            $menu_list['trade']['child']['member_order']  = array('name' => '订单查询', 'url'=>urlShop('member_order_third', 'index'));
            $menu_list['serv']['child']['member_complain']  = array('name' => '交易投诉', 'url'=>urlShop('member_complain', 'index'));
        }else{
            $menu_list['trade']['child']['member_order']  = array('name' => '实物交易订单', 'url'=>urlShop('member_order', 'index'));
            /*$menu_list['trade']['child']['member_vr_order']  = array('name' => '虚拟兑码订单', 'url'=>urlShop('member_vr_order', 'index'));*/
        }         
    ...
    private function filter_menu($menu_list){
    	...
    	if($setting['flea_isuse']=='0'){
            unset($menu_list['trade']['child']['member_flea']);
            unset($menu_list['app']['child']['member_flea']); // add flea
        }
        if($setting['yungou_isuse']=='0') {
            unset($menu_list['trade']['child']['yungou']);
            unset($menu_list['app']['child']['yungou']);
        }
...
class BaseGoodsControl extends BaseStoreControl {
    ...
    protected function getStoreInfo($store_id, $goods_info = null) {
        ...
        $store = explode(',',$store_info['store_city_id']);
        if($city){
            foreach($city as $c_id){
                foreach($store as $s_id){
                    if($c_id['id'] == $s_id){
                        $store_name .= $c_id['city_name'].',';
                    }
                }
            }
        }

        $store_info['store_city_name'] = substr($store_name, 0,-1);
        if(empty($store_info)) {
            showMessage(L('nc_store_close'), '', '', 'error');
        }
        if ($_COOKIE['dregion']) {
...
class BaseSellerControl extends Control {
    ...
    public function __construct(){
        ...
        if ($_GET['act'] !== 'seller_login') {
            ...
            //校验session有限时长     //写入session有效期   @Aletta @time 2017.05.23 10:40
...
//                array('name' => '修改商品库存', 'act'=>'store_goods_stock', 'op'=>'index'),
                ...
                array('name' => '基本信息申请', 'act'=>'attribute_add', 'op'=>'index'),
            )),
            'order' => array('name' => '订单物流', 'child' => array(
                
                array('name' => '万科物业订单', 'act'=>'store_transport_nei', 'op'=>'index'),
                array('name' => '其他物业订单', 'act'=>'store_order', 'op'=>'index'),
...
//                array('name' => '佣金结算', 'act'=>'store_commission', 'op'=>'index'),   //cary_add 店铺 佣金菜单
            )),
 ...
 *
 */
class BaseAgentControl extends Control {
    ...
    public function __construct(){
        ...
        Tpl::output('nav_list', rkcache('nav',true));
        if ($_GET['act'] !== 'agent_login') {

            if(empty($_SESSION['agent_id'])) {
                @header('location: index.php?act=agent_login&op=show_login');die;
            }



                $store_grade = rkcache('agent_grade', true);
                $this->store_grade = $store_grade[$this->store_info['grade_id']];
    ...
    private function _getMenuList() {
        $menu_list = array(
            'order' => array('name' => '管理中心', 'child' => array(
                array('name' => '交易订单', 'act'=>'agent_order', 'op'=>'index'),
                array('name' => '店铺管理', 'act'=>'agent_store', 'op'=>'index'),
                array('name' => '佣金结算', 'act'=>'agent_commission', 'op'=>'index'),
            ))
        );
        return $menu_list;
    }
