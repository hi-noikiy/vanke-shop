...
//         Tpl::output('role_id', $member_list['role_id']);      
     ...
     * @Aletta 2017.06.06
     **/
    protected function getPurchasingAuthority($good_id,$member_id){
        $authority = '2';
        $model = Model();
        if(!empty($member_id)){
            //判定用户是否登录
    ...
    public function salelogOp() {
        ...
        $role_id=$model_member = Model('member')->field('role_id')->where(array('member_id'=>$_SESSION['member_id']))->find();
        Tpl::output('role_id',$role_id['role_id']);
