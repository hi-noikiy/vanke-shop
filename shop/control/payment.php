...
//print_r($order_info);
    	...
    	if($payment_info['payment_code'] == 'chinabank') {
            ...
            if (array_key_exists('order_list', $order_info)) {
            ...
            } elseif(array_key_exists('order_id', $order_info)) {
				...
				Tpl::output('order_list',array($order_info));
				Tpl::output('args','member_id='.$_SESSION['member_id'].'&pdr_id='.$order_info['pdr_id']);
			}
    ...
    public function query_stateOp() {
        ...
        } elseif (intval($_GET['pdr_id']) > 0) {// add cary 充值
//			pd_recharge
			$info = Model('predeposit')->getPdRechargeInfo(array('pdr_id'=>intval($_GET['pdr_id']),'pdr_member_id'=>intval($_GET['member_id'])));
