...
class searchControl extends BaseHomeControl {
    ...
    public function indexOp() {
        ...
        if (intval($_GET['cate_id']) > 0) {
            $goods_class_arrays  = $this->_model_search->getLeftCategory(array($_GET['cate_id']));
            $goods_class_array = $goods_class_arrays[0];
            $goods_class_parent_array = $goods_class_arrays[1];
        } elseif ($_GET['keyword'] != '') {
            //从TAG中查找分类
            ...
            $goods_class_arrays  = $this->_model_search->getLeftCategory($goods_class_array, 1);
            $goods_class_array = $goods_class_arrays[0];
            $goods_class_parent_array = $goods_class_arrays[1];
        }
        Tpl::output('goods_class_array', $goods_class_array);
        Tpl::output('goods_class_parent_array', $goods_class_parent_array);
        ...
        if (is_array($indexer_ids)) {
            ...
            if(!empty($_SESSION['ctid'])){
//                $condition['sales_area']=$_SESSION['ctid'];
                $sqlcon.= "AND ( CONCAT(',',sales_area,',') like '%,".$_SESSION['ctid'].",%'  or ( CONCAT(',',sales_area,',') like '%,1,%' ) )";
            }
            //执行正常搜索
            if (isset($goods_param['class'])) {
//                $condition['gc_id_'.$goods_param['class']['depth']] = $goods_param['class']['gc_id'];
                $sqlcon.=" AND ( ".'gc_id_'.$goods_param['class']['depth']." = ". $goods_param['class']['gc_id'].' ) ';
            }
            if (intval($_GET['b_id']) > 0) {
                // $condition['brand_id'] = intval($_GET['b_id']);
                $sqlcon.=" AND ( brand_id  = ".intval($_GET['b_id'])." )";  
            }
            if ($_GET['keyword'] != '') {
                  //$condition['goods_name|goods_jingle'] = array('like', '%' . $_GET['keyword'] . '%');
                 $sqlcon.=" AND ( goods_name like '%".$_GET['keyword']."%' ) ";  
            }
            if (intval($_GET['area_id']) > 0) {
//                $condition['areaid_1'] = intval($_GET['area_id']);
                 $sqlcon.=" AND (  area_id  =  ".$_GET['keyword']." ) "; 
            }
            if ($_GET['type'] == 1) {
//                $condition['is_own_shop'] = 1;
                 $sqlcon.=" AND ( is_own_shop = 1 ) "; 
            }
            if ($_GET['gift'] == 1) {
//                $condition['have_gift'] = 1;
                 $sqlcon.=" AND ( have_gift = 1 ) "; 
            }
            if (isset($goods_param['goodsid_array'])){
//                $condition['goods_id'] = array('in', $goods_param['goodsid_array']);
                  $sqlcon.=" AND ( goods_id in ".$goods_param['goodsid_array']." ) "; 
            }
            //获取到seesion中的城市公司id  or 

            //商品输出
            $goods_list = $model_goods->getGoodsListByColorDistinct2($sqlcon, $fields, $order, self::PAGESIZE);
        }
		...
		$n=0;
                ...
                $city_id = explode(',',$store_list[$store_id]['store_city_id']);
                if($cache_info_list){
                    foreach($city_id as $c_id){
                        foreach($cache_info_list as $a_id){
                            if($c_id == $a_id['id']){
                                $store_name .= $a_id['city_name'].",";
                            }
                        }
                    }
                }
                
                $store_name = substr($store_name, 0,-1);
                $goods_list[$key]['store_city_name'] = $store_name;
                unset($store_name);
                //将关键字置红
                if ($search_keyword){
    ...
    public function get_hot_goodsOp() {
        ...
        $fieldstr = "goods_id,goods_commonid,goods_name,goods_jingle,store_id,store_name,goods_price,goods_promotion_price,goods_promotion_type,goods_marketprice,goods_storage,goods_image,goods_freight,goods_salenum,color_id,evaluation_count,g_costprice";
