...
/*    public function cityListOp(){
     ...
     * @Time    : 2017-11-23 PM 14:02
     */
    public function provinceListOp(){
        $list = Model()->table("linkage_province")->where()->select();
        $city_json = json_encode($list);
        exit($city_json);
    }
    ...
    public function cityListOp(){
        $parent_id = isset($_GET["parent_id"]) ? $_GET["parent_id"] : "0";
        $where = "belong_province_code = '".$parent_id."'";
        $list = Model()->table("linkage_city")->where($where)->select();
        $city_json = json_encode($list);
        exit($city_json);
    }
     ...
     * @Time    : 2017-11-23 PM 14:02
     */
    public function countyListOp(){
        $parent_id = isset($_GET["parent_id"]) ? $_GET["parent_id"] : "0";
        $where = "belong_city_code = '".$parent_id."'";
        $list = Model()->table("linkage_county")->where($where)->select();
        $city_json = json_encode($list);
        exit($city_json);
    }
     ...
     * @Time    : 2017-11-23 PM 14:02
     */
    public function streetListOp(){
        $parent_id = isset($_GET["parent_id"]) ? $_GET["parent_id"] : "0";
        $where = "belong_province_code = '".$parent_id."'";
        $list = Model()->table("linkage_street")->where($where)->select();
        $city_json = json_encode($list);
        exit($city_json);
    }
