...
class member_inorderControl extends BaseMemberControl {
     ...
     *
     */
    public function inside_orderOp() {
        Language::read('member_member_index');
        $lang	= Language::getLangContent();
        $model_order = Model('order');
        //如果是返回的订单返回当前订单号给前端
        if($_GET['member_num']){
            Tpl::output('order_num',$_GET['member_num']);
        }
        //搜索
        $condition = array();
        ...
        if ($_GET['state_type'] != '') {
            $condition['order_state'] = str_replace(
                    array('state_uncheck','state_checking','state_checked','state_checkno'),
                    array(ORDER_STATUS_SEND_ONE,ORDER_STATUS_SEND_TWO,ORDER_STATUS_SUCCESS,ORDER_STATUS_ERROR), $_GET['state_type']);
        }
        ...
        foreach ($order_list as $order_id => $order) {
            ...
            //显示退货
            $order['check_back'] = $model_order->getOrderOperateState('check_back',$order);
            
            //退货中
            $order['if_back'] = $model_order->getOrderOperateState('back',$order);
	...
	self::profile_menu($_GET['recycle'] ? 'member_order_recycle' : 'member_order');
        Tpl::showpage('member_order.inside');
    ...
    public function show_orderOp() {
		...
		Tpl::showpage('member_inorder.show');
    }
