...
class member_order_thirdControl extends BaseMemberControl {
    ...
    public function indexOp() {
        ...
        //新增判断是否是售后退款，
        foreach($order_group_list as $o_one){
            $order_refund_id .= $o_one['order_list'][0]['order_id'].',';
        }
        //查询申请退款中的订单
        $condition_orderfund['order_id'] = array('in',$order_refund_id);
        $condition_orderfund['refund_state'] = '3';
        $condition_orderfund['goods_id'] = array('gt',0);
        $refund_list = $model_refund_return->where($condition_orderfund)->field('order_id,goods_id,refund_id,refund_type')->select();
        foreach($refund_list as $o_r_two){
            foreach($order_group_list as $key=>$o_one_r){
                if($o_one_r['order_list'][0]['order_id'] == $o_r_two['order_id']){
                    foreach($o_one_r['order_list'][0]['goods_list'] as $key_godos=>$o_g_one){
                        if($o_g_one['goods_id'] == $o_r_two['goods_id']){
                            $order_group_list[$key]['order_list'][0]['goods_list'][$key_godos]['refund_type'] = $o_r_two['refund_type'];
                            $order_group_list[$key]['order_list'][0]['goods_list'][$key_godos]['is_refund_onoall'] = $o_r_two['refund_id'];
                        }
                    }
                }
            }
        ...
        Tpl::showpage('member_order.third');
    }
