...
<div class="breadcrumb"><span class="icon-home"></span><span><a href="<?php echo SHOP_SITE_URL;?>">首页</a></span> <span class="arrow">></span> <span>供应商认证申请</span> </div>
<div class="main">
  <div class="sidebar">
    <div class="title">
      <h3>供应商认证申请</h3>
    ...
    <div class="content">
      ...
      <?php } ?>
      <dl>
        <dt class="<?php echo $output['sub_step'] == 'step0' ? 'current' : '';?>"> <i class="hide"></i>签订认证协议</dt>
      </dl>
      <dl show_id="0">
...
<!--      <dl>
  ...
  <div class="right-layout">
    ...
    <div class="joinin-concrete">
     <table border="1" style="text-align:center;width:100%;" cellpadding="1">
	<tbody>
	<tr>
            <td>
        ...
        <?php foreach($output['log'] as $rows){?>
        <tr>
            <td>
                <h2 style="text-align:center;">
            ...
            <td>
                <h2 style="text-align:center;">
                    <span><?php echo $rows['contacts_name']?></span> 
                </h2>
            </td>
            <td>
                <h2 style="text-align:center;">
                    <span>
                        <?php if($rows['joinin_state'] == STORE_JOIN_STATE_RZ){?>
                        认证中
                        <?php }else if($rows['joinin_state'] == STORE_JOIN_STATE_RZSUCCESS){?>
                        认证成功
                        <?php }else if($rows['joinin_state'] == STORE_JOIN_STATE_EMAIL){?>
                        未认证邮箱
                        <?php }else if($rows['joinin_state'] == STORE_JOIN_STATE_FNO){?>
                        认证拒绝
                        <?php }else{?>
                        认证回退
                        <?php }?>
            ...
            <td>
                <h2 style="text-align:center;">
                    <?php if($rows['city_center'] == $rows['first_city_id']){?>
                    <span><a style="color:red" href="<?php echo BASE_SITE_URL;?>/shop/index.php?act=storejoininlog&op=logedit&id=<?php echo $rows['city_center'];?>">编辑</a></span> 
                    <?php }?>
                    <span><a href="<?php echo BASE_SITE_URL;?>/shop/index.php?act=storejoininlog&op=loglook&id=<?php echo $rows['city_center'];?>">查看</a></span> 
                </h2>
            </td>
	</tr>
        <?php }?>
