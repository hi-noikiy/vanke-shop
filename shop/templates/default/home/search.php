    ...
    <div class="nch-module nch-module-style02">
      ...
      <div class="content">
        <ul id="files" class="tree">
          ...
          <li id="cm_<?php echo $value['gc_id']; ?>"><i class="tree-parent tree-parent-collapsed"></i><a href="<?php echo urlShop('search', 'index', array('cate_id' => $value['gc_id'], 'keyword' => $_GET['keyword']));?>" <?php if ($value['gc_id'] == $_GET['cate_id']) {?>class="selected"<?php }?>><?php echo $value['gc_name']?></a>
            <?php if (!empty($value['class2'])) {?>
            <ul>
              <?php foreach ($value['class2'] as $val) {?>
              <li id="cm_<?php echo $val['gc_id']; ?>"><i class="tree-parent tree-parent-collapsed"></i><a href="<?php echo urlShop('search', 'index', array('cate_id' => $val['gc_id'], 'keyword' => $_GET['keyword']));?>" <?php if ($val['gc_id'] == $_GET['cate_id']) {?>class="selected"<?php }?>><?php echo $val['gc_name']?></a>
                <?php if (!empty($val['class3'])) {?>
                <ul>
                  <?php foreach ($val['class3'] as $v) {?>
                  <li id="cm_<?php echo $v['gc_id']; ?>" class="tree-parent tree-parent-collapsed"><i></i><a href="<?php echo urlShop('search', 'index', array('cate_id' => $v['gc_id'], 'keyword' => $_GET['keyword']));?>" <?php if ($v['gc_id'] == $_GET['cate_id']) {?>class="selected"<?php }?>><?php echo $v['gc_name']?></a></li>
                  <?php }?>
                </ul>
                <?php }?>
              </li>
...
$(function(){
    $('#files').tree({
       // expanded: 'li:lt(2)'
      expanded: '<?php if (!empty($output['goods_class_parent_array'])) {
  echo "#cm_".join(",#cm_",$output['goods_class_parent_array']);
   } ?>'
    });
  //品牌索引过长滚条
