  ...
  <div class="alert">
    ...
    <form id="form_store_info" action="index.php?act=store_joinin_c2c&op=step4" method="post" >
      <table border="0" cellpadding="0" cellspacing="0" class="all">
        ...
        <tr>
            <th><i>*</i>店铺类型：</th>
            <td><select name="st_id" id="st_id">
                    <option value="">请选择</option>
                    <?php if(!empty($output['store_type']) && is_array($output['store_type'])){ ?>
                        <?php foreach($output['store_type'] as $k => $v){ ?>
                            <option value="<?php echo $v['st_id'];?>" ><?php echo $v['st_name'];?></option>
                        <?php } ?>
                    <?php } ?>
                </select>
                <input id="st_name" name="st_name" type="hidden" />
                <span></span>
                <p class="emphasis">请认真选择店铺类型，注册后商家不可自行修改。</p></td>
        </tr>
        ...
        <?php if(C('store_class_bind_isuse')){ ?>
         <th></th>
          <td><span id="jylm_str"></span>
          <input id="store_class" name="store_class" type="hidden" />
            <input name="store_class_ids[]" id="store_class_ids_0" type="hidden" value="">
            <input name="store_class_names[]" id="store_class_names_0" type="hidden" value="">
          </td>
        </tr>
        <?php }else{ ?>
        <tr>
      ...
      <tfoot>
        ...
        <table border="0" cellpadding="0" cellspacing="0" class="all">
            <thead>
            <tr>
                <th colspan="20">代理商编号</th>
            </tr>
            </thead>
            <tbody>
            <tr>
                <th class="w150"><i>*</i>代理编号：</th>
                <td><input id="agent_id" name="agent_id" type="text" class="w200"/>
                    <span></span>
                    <p class="emphasis">没有可以不用填写。</p></td>
            </tr>
        </tbody>
            </table>
  </form>
