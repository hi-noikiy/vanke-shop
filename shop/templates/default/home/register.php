      ...
      <div class="left">
        <div class="nc-register-mode">
          <ul class="tabs-nav">                 
            ...
            <?php } ?>  
                <li style="display: none"><a href="#default">邮箱注册<i></i></a></li>        
          </ul>
	      ...
	      <form style="display: none;" id="register_sms_form" class="nc-login-form" method="post" action="index.php?act=connect_sms&op=register">
                <input type="hidden" name="form_submit" value="ok" />
                ...
                <dl>
                  ...
                  <dd>
                    <input type="text" id="member_name" name="member_name" class="text w150" value="" readOnly="true"/>
                  </dd>
                  <span class="note">用户名默认为手机号，不可修改。</span>
                </dl>
                <dl>
                  ...
                  <span class="note">请设置6-20位密码。</span>
                </dl>
                <dl>
                  <dt><?php echo $lang['login_register_ensure_password'];?>：</dt>
                  <dd>
                    <input type="password" id="sms_password_chekc" name="passwordcheck" class="text w150" value=""/>
                  </dd>
                  <span class="note">请再次输入密码。</span>
                </dl>
                <dl class="mt15" id="mt15">
                  <dt><?php echo $lang['login_register_email'];?>：</dt>
                  <dd>
                    <input type="text" id="sms_email" name="email" class="text w150" value=""  />
                  </dd>
                </dl>
                <dl class="clause-div">
                  <dd>
            ...
            <?php } ?>
<!--邮箱注册注释            <div id="default" class="tabs-content">
                <form id="post_form_email" method="post" class="nc-login-form">
                <?php Security::getToken();?>
                <input type="hidden" name="form_submit" value="ok" />
                <input name="nchash" type="hidden" value="<?php echo getNchash();?>" />
                <dl>
                  <dt>邮箱号：</dt>
                  <dd>
                    <input type="text" class="text" tipMsg="请输入邮箱号码" autocomplete="off" value="" name="loginEmail" id="loginEmail"  >
                  </dd>
                </dl>
                <div class="code-div">
                  <dl>
                    <dt><?php echo $lang['login_register_code'];?>：</dt>
                    <dd>
                      <input type="text" name="captcha" class="text w100" id="image_captcha_email" size="10" tipMsg="<?php echo $lang['login_register_input_code'];?>" />
                    </dd>
                  </dl>
                  <span><img src="index.php?act=seccode&op=makecode&type=50,120&nchash=<?php echo getNchash();?>" title="<?php echo $lang['login_index_change_checkcode'];?>" name="codeimage" id="email_codeimage"><a class="makecode" href="javascript:void(0);" onclick="javascript:document.getElementById('email_codeimage').src='index.php?act=seccode&op=makecode&type=50,120&nchash=<?php echo getNchash();?>&t=' + Math.random();"><?php echo $lang['login_password_change_code']; ?></a></span> </div>
                
                <div class="tiptext" id="email_text">确保上方验证码输入正确，点击<span><a href="javascript:void(0);" onclick="get_email_captcha()"><i class="icon-mobile-phone"></i>发送邮件验证</a></span>，并将您邮件所接收到的“6位动态码”输入到下方邮件验证，再提交下一步。</div>
                <dl>
                  <dt>邮件验证：</dt>
                  <dd>
                    <input type="text" name="email_captcha" autocomplete="off" tipMsg="输入6位邮件验证码" class="text" id="email_captcha" size="15" />
                  </dd>
                </dl>
                <div class="submit-div">
                  <input type="button" id="emailSubmitBtn" class="submit" value="下一步">
                </div>
              </form>
                <form style="display: none" id="register_form" class="nc-login-form"  method="post" action="<?php echo SHOP_SITE_URL;?>/index.php?act=login&op=usersave">
                <?php Security::getToken();?>
                <dl>
                  <dt><?php echo $lang['login_register_username'];?>：</dt>
                  <dd>
                      <input type="text" id="user_name" name="user_name" class="text" tipMsg="<?php echo $lang['login_register_username_to_login'];?>" readOnly="true" style="background-color:gary"/>
                  </dd>
                </dl>
                <dl>
                  <dt><?php echo $lang['login_register_pwd'];?>：</dt>
      ...
      <div class="right">
        <?php if (C('qq_isuse') == 1 || C('sina_isuse') == 1 || C('weixin_isuse') == 1){?>
          <div class="api-login" style="display: none">
          <h4>使用合作网站账号直接登录</h4>
          <?php if (C('qq_isuse') == 1){?>
...
$(function(){
	...
	document.getElementById("mt15").style.display="none";
        //切换邮件验证和下一步
...
//注册表单验证
    $("#register_form").validate({
        ...
        messages : {
            user_name : {
                ...
                maxlength : '<i class="icon-exclamation-sign"></i><?php echo '邮箱号必须在3-50位字符之间';?>',
    ...
    //这里添加邮件注册发送邮件前的输入框限制条件
    $("#post_form_email").validate({
        errorPlacement: function(error, element){
            var error_td = element.parent('dd');
            error_td.append(error);
            element.parents('dl:first').addClass('error');
		...
		rules: {
			...
			captcha : {
                            ...
                            remote   : {
                                    ...
                                    data:{
                                        captcha : function(){
                                        return $('#image_captcha_email').val();
                                        }
                                    },
                                    complete: function(data) {
                                        if(data.responseText == 'false') {
                                                document.getElementById('email_codeimage').src='index.php?act=seccode&op=makecode&type=50,120&nchash=<?php echo getNchash();?>&t=' + Math.random();
                                        }
                                    }
                             }
                        },
			email_captcha: {
                            ...
                            return $("#image_captcha_email").val().length == 4;
                        },
                        minlength: 6
                        }
		},
		messages: {
			loginEmail: {
                            required : '<i class="icon-exclamation-sign"></i>输入正确的邮箱号',
                            email : '<i class="icon-exclamation-sign"></i>输入正确的邮箱号'
                        },
                        captcha : {
                            required : '<i class="icon-remove-circle" title="<?php echo $lang['login_register_input_text_in_image'];?>"></i>',
                        ...
                        email_captcha: {
                            required : '<i class="icon-exclamation-sign"></i>请输入六位邮件动态码',
                            minlength: '<i class="icon-exclamation-sign"></i>请输入六位邮件动态码'
                        }
		}
	});
});
...
$(function(){
    ...
    $('#register_sms_form').validate({
        ...
        //添加正则表达式 匹配密码至少包含特殊字符，小写字母，大写字母或数字
     $.validator.addMethod("password",function(value,element,params){  
            var checkName =/^(?![A-Z]+$)(?![a-z]+$)(?!\d+$)(?![\W_]+$)\S+$/;  
            return this.optional(element)||(checkName.test(value));  
        },"<i class='icon-exclamation-sign'></i>至少包含特殊字符，小写字母，大写字母或数字中的两种");
});
</script>
<?php } ?>
