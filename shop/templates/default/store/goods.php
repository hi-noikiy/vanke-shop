      ...
      <div class="ncs-buy">
        ...
        <?php if ($output['goods']['goods_state'] == 0 || $output['goods']['goods_storage'] <= 0) {?>
            <?php if($output['goods']['goods_state'] == 0){?>
                <div nctype="goods_prompt" class="ncs-point"><i></i> <span>您选择的商品<strong>已经下架</strong>；请选择店内其它商品</span> </div>
            <?php }else{ ?>
        ...
        <div class="ncs-btn"> 
          ...
          <?php if($output['goods']['goods_state'] == 0){ ?>
              ...
              <!--  限制购买区域-->
          <?php }else if(empty(array_intersect(explode(',',$_SESSION['city_id']),explode(',',$output['goods']['sales_area'])))){?>
    ...
    $('a[nctype="addcart_submit"]').click(function(){
        ...
        $('a[nctype="buynow_submit"]').click(function(){
            <?php if($_SESSION['identity'] != MEMBER_IDENTITY_TWO){?>
                <?php if($_SESSION['identity'] == MEMBER_IDENTITY_THREE || $_SESSION['identity'] == MEMBER_IDENTITY_FOUR ){?>
                alert('对不起，您没有权限购买商品！');return false;
                <?php } ?>
            <?php  }?>
            $.post(
            SITEURL + '/index.php?act=cart&op=get_goodsnum_api',
            {
                'goods_id':<?php echo $output['goods']['goods_id'];?>
            },
                function(data){
