...
<div class="cms-sns">
  ...
  <div class="cms-sns-right">
    ...
    <?php if (!empty($output['favorites_list']) && false) {?>
    <div class="cms-sns-right-container">
      <div class="title">最新收藏用户</div>
      <div class="cms-favorites-user">
        <ul>
          <?php foreach ($output['favorites_list'] as $val) {?>
          <li><a target="_blank" href="javascript:void(0);"><img alt="<?php echo $val['member_name'];?>" title="<?php echo $val['member_name'];?>" src="<?php echo getMemberAvatarForID($val['member_id']);?>" /></a></li>
          <?php }?>
        </ul>
      </div>
    </div>
