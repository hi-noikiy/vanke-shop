...
<div class="ncc-main">
  <div class="ncc-title">
    ...
    <h5>查看购物车商品清单，增加减少商品数量。</h5>
  </div>
  <!--20170410 一句需求，不进入check_card_num步骤，直接进入核对生成订单 因此注释下行使用直接跳转到buy-->
 <!-- <form action="<?php if($_SESSION['city_id']){ echo urlShop('cart','check_card_num');}else{echo  urlShop('buy','buy_step1');}?>" method="POST" id="form_buy" name="form_buy">   -->
      ...
      <tbody>
        <tr>
          <th colspan="20"><strong>店铺：<a href="<?php echo urlShop('show_store','index',array('store_id'=>$store_id), $output['store_list'][$store_id]['store_domain']);?>"><?php echo $cart_list[0]['store_name']; ?></a></strong> <span member_id="<?php echo $output['store_list'][$store_id]['member_id'];?>"></span>
            ...
            <div class="store-sale"><strong>满<span style="color:red"><?php echo $output['free_freight_list'][$store_id];?></span>免运费</strong>&emsp;</div>
            <?php } ?>
          </th>
        </tr>
        ...
        <tr id="cart_item_<?php echo $cart_info['cart_id'];?>" nc_group="<?php echo $cart_info['cart_id'];?>" class="shop-list <?php echo $cart_info['state'] ? '' : 'item_disabled';?>">
          <td>
          <input type="checkbox" <?php echo $cart_info['state'] ? 'checked' : 'disabled';?> nc_type="eachGoodsCheckBox" value="<?php echo $cart_info['cart_id'].'|'.$cart_info['goods_num'];?>" id="cart_id<?php echo $cart_info['cart_id'];?>" name="cart_id[]">
          </td>
          ...
          <td class="w120 ws0"><a href="JavaScript:void(0);" onclick="decrease_quantity(<?php echo $cart_info['cart_id']; ?>,<?php echo $cart_info['good_min']; ?>);" title="<?php echo $lang['cart_index_reduse'];?>" class="add-substract-key tip">-</a>
            <input id="input_item_<?php echo $cart_info['cart_id']; ?>" value="<?php echo $cart_info['goods_num']; ?>" orig="<?php echo $cart_info['goods_num']; ?>" changed="<?php echo $cart_info['goods_num']; ?>"
                   onkeyup="change_quantity(<?php echo $cart_info['cart_id']; ?>, this);"
                   onblur="this.value=this.value.replace(/\D/g,'')"
                   style="width:70px;" type="text" class="text w20"/>
            <a href="JavaScript:void(0);" onclick="add_quantity(<?php echo $cart_info['cart_id']; ?>,<?php echo $cart_info['good_max']; ?>,<?php echo $cart_info['goods_storage']; ?>);"
               style="float: left;margin-left: 130px;margin-top: -20px;" title="<?php echo $lang['cart_index_increase'];?>" class="add-substract-key tip" >+</a></td>
          <?php } else {?>
          <td class="w120">无效
