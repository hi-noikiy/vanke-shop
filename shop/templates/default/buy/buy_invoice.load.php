...
<!--              原本是隐藏的现在给他默认显示style="display:none"-->
        ...
        <dl id="inv_code">
          ...
          <dd>
            <input type="text" class="text w200" maxlength="50" name="inv_code1"  value="">
          </dd>
        </dl>
        ...
        <dl>
          ...
          <dd>
            <input type="text" class="text w200" maxlength="50" name="inv_rec_name1" value="">
          </dd>
        </dl>
        <dl>
          <dt><i class="required">*</i>收票人手机号<?php echo $lang['nc_colon'];?></dt>
          <dd>
            <input type="text" class="text w200" maxlength="50" name="inv_rec_mobphone1" value="">
          </dd>
        </dl>
        <dl>
          ...
          <dd id="region">
            ...
            <input type="hidden" value="" name="city_id1" id="city_id">
            <input type="hidden" name="area_id1" id="area_id" class="area_ids"/>
            <input type="hidden" name="area_info1" id="area_info" class="area_names"/>
          </dd>
        </dl>
        <dl>
          <dt><i class="required">*</i>收票地址<?php echo $lang['nc_colon'];?></dt>
          <dd>
            <input type="text" class="text w200" maxlength="50" name="inv_goto_addr1" value="">
          </dd>
        </dl>
...
$(function(){
    ...
    $('#hide_invoice_list').on('click',function(){
        ...
        if ($('input[name="inv"]:checked').size() == 0){
            //注释点击不适用发票 改为提示必须选择发票
        	//$('#cancel_invoice').click();
                alert("发票必须选择，请添加新发票或选择一个发票信息！");
        	return false;
        }
        ...
        if ($('input[name="invoice_type"]:checked').val() == 1){
            ...
            if ($('select[name="inv_title_select"]').val() == 'person'){
                content = '增值税普通发票 个人 ' + $('select[name="inv_content"]').val();
            }else if($.trim($('#inv_title').val()) == '' || $.trim($('#inv_title').val()) == '单位名称'){
                showDialog('请填写单位名称', 'error','','','','','','','','',2);return false;
            }else{
                content = '增值税普通发票 ' + $.trim($('#inv_title').val())+ ' ' + $('#inv_content').val();
            }
        }else{
            content = '增值税专用发票 ' + $.trim($('input[name="inv_company"]').val()) + ' ' + $.trim($('input[name="inv_code"]').val()) + ' ' + $.trim($('input[name="inv_reg_addr"]').val());
            //验证增值税专用发票表单
            if (!$('#inv_form').valid()){
                return false;
            }
