...
<div class="ncc-main">
  <div class="ncc-title">
    ...
    <h5>查看购物车商品清单，并勾选想要的商品进入下一步操作。</h5>
  </div>
    ...
    <table class="ncc-table-style" nc_type="table_cart">
      ...
      <tbody>
        ...
        <tr id="cart_item_<?php echo $cart_info['cart_id'];?>" nc_group="<?php echo $cart_info['cart_id'];?>" class="shop-list <?php echo $cart_info['state'] ? '' : 'item_disabled';?>">
          ...
          <td class="w120 ws0">
            <input id="input_item_<?php echo $cart_info['cart_id']; ?>" disabled value="<?php echo $cart_info['goods_num']; ?>" orig="<?php echo $cart_info['goods_num']; ?>" changed="<?php echo $cart_info['goods_num']; ?>" onkeyup="change_quantity(<?php echo $cart_info['cart_id']; ?>, this);" type="text" class="text w20"/>
           </td>
          <?php } else {?>
          <td class="w120">无效
          ...
          <td class="w80"><?php if ($cart_info['bl_id'] == '0') {?>
              <a href="javascript:void(0)" <?php if($cart_info['materiel_now'] > $cart_info['materiel_min']){?> <?php }?> >当前库存&nbsp;<strong><?php echo $cart_info['materiel_now'];?></strong></a><br/>
            <?php } ?>
              <a href="javascript:void(0)" <?php if($cart_info['materiel_now'] > $cart_info['materiel_min']){?> <?php }?> >最低库存&nbsp;<strong><?php echo $cart_info['materiel_min']; ?></strong></a></td>
        </tr>
  ...
  <div class="ncc-bottom">
      <a id="next_submit2" style="background-color:#49AF9D;border-color: #49AF9D #49AF9D #49AF9D #49AF9D;" href="javascript:history.go(-1);" class="ncc-btn ncc-btn-acidblue fr"><i class="icon-pencil"></i>取消购买</a>
        
      <a id="next_submit2" href="javascript:void(0)" class=" fr">&nbsp;</a>

      <a id="next_submit" href="javascript:void(0)" class="ncc-btn ncc-btn-acidblue fr"><i class="icon-pencil"></i>继续购买</a>
  </div>
