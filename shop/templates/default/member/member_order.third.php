...
<div class="tabmenu">
<ul class="tab pngFix">
  <li class="<?php if($_GET['recycle'] == 1){echo 'normal';}else{echo 'active';}?>" ><a href="index.php?act=member_order_third">订单列表</a>
  </li><li class="<?php if($_GET['recycle'] == 1){echo 'active';}else{echo 'normal';}?>"><a href="index.php?act=member_order_third&recycle=1">回收站</a></li>
</ul>

  </div>
  <form method="get" action="index.php" target="_self">
    <table class="ncm-search-table">
      <input type="hidden" name="act" value="member_order_third" />
    ...
    <tbody order_id="" <?php if (!empty($group_info['pay_amount']) && $p == 0) {?> class="pay" <?php }?>>
      ...
      <tr>
        <th colspan="19"> <span class="ml10">
          ...
          <a href="javascript:void(0);" class="order-trash" onclick="ajax_get_confirm('您确定要删除吗?删除后该订单可以在回收站找回，或彻底删除', 'index.php?act=member_order_third&op=change_state&state_type=order_delete&order_id=<?php echo $order_info['order_id']; ?>');"><i class="icon-trash"></i>删除</a>
          <?php } ?>

          <!-- 还原订单 -->

          <?php if ($order_info['if_restore']) { ?>
          <a href="javascript:void(0);" class="order-trash" onclick="ajax_get_confirm('您确定要还原吗?', 'index.php?act=member_order_third&op=change_state&state_type=order_restore&order_id=<?php echo $order_info['order_id']; ?>');"><i class="icon-refresh"></i>还原</a>
          <?php } ?>
        </th>
      </tr>
      ...
      <tr>
        ...
        <td><!-- 退款 -->
            <?php if($goods_info['is_refund_onoall'] > 0){
                if($goods_info['refund_type'] == 1){
            ?>
            <p><a href="index.php?act=member_refund&op=view&refund_id=<?php echo $goods_info['is_refund_onoall'];?>">查看退货/退款</a></p>
                <?php }else{?>
            <p><a href="index.php?act=member_return&op=view&return_id=<?php echo $goods_info['is_refund_onoall'];?>">查看退货/退款</a></p>
        ...
        <td class="bdl" rowspan="<?php echo $order_info['goods_count'];?>">
            ...
            <p><?php echo $order_info['state_desc']; ?> <?php echo $order_info['evaluation_status'] ? $lang['member_order_evaluated'] : '';?></p>
           
          <!-- 订单查看 -->
          
          <p><a href="index.php?act=member_order_third&op=show_order&order_id=<?php echo $order_info['order_id']; ?>" target="_blank"><?php echo $lang['member_order_view_order'];?></a></p>

          <!-- 物流跟踪 -->

          <?php if ($order_info['if_deliver']){ ?>
          <p><a href='index.php?act=member_order_third&op=search_deliver&order_id=<?php echo $order_info['order_id']; ?>&order_sn=<?php echo $order_info['order_sn']; ?>' target="_blank"><?php echo $lang['member_order_show_deliver']?></a></p>
          <?php } ?></td>
        <td class="bdl bdr" rowspan="<?php echo $order_info['goods_count'];?>"><!-- 永久删除 -->
          ...
          <p><a href="javascript:void(0);" onclick="ajax_get_confirm('您确定要永久删除吗?永久删除后您将无法再查看该订单，也无法进行投诉维权，请谨慎操作！', 'index.php?act=member_order_third&op=change_state&state_type=order_drop&order_id=<?php echo $order_info['order_id']; ?>');" class="ncm-btn ncm-btn-red mt5"><i class="icon-trash"></i>永久删除</a></p>
          <?php } ?></td>
        <!-- E 合并TD -->
        <?php } ?>
      </tr>
