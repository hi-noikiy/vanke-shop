...
<div class="item-publish">
  <form method="post" id="goods_form" action="<?php if ($output['edit_goods_sign']) { echo urlShop('store_goods_online', 'edit_save_goods');} else { echo urlShop('store_goods_add', 'save_goods');}?>">
    ...
    <div class="ncsc-form-goods">
      ...
      <style type="text/css">.red_blur {border: 1px solid red;} </style>
          ...
          <table border="0" cellpadding="0" cellspacing="0" class="spec_table" style="margin:0 0">
            ...
            <th class="w90"><span class="red">*</span>协议价(元)
                <div class="batch"><i class="icon-edit" title="批量操作"></i>
                  <div class="batch-input" style="display:none;">
                    ...
                    <input name="" type="text" class="text price" pctype="pice_num" />
                    <a href="javascript:void(0)" class="ncsc-btn-mini" data-type="marketprice">设置</a><span class="arrow"></span>
                  </div>
                </div>
              </th>
              ...
              <th class="w60"><span class="red">*</span><?php echo $lang['store_goods_index_stock'];?>
                <div class="batch"><i class="icon-edit" title="批量操作"></i>
                  <div class="batch-input" style="display:none;">
                    <h6>批量设置库存：</h6>
                    <a href="javascript:void(0)" class="close">X</a>
                    <input name="" type="text" class="text stock" onkeyup="this.value=this.value.replace(/\D/g,'')" onafterpaste="this.value=this.value.replace(/\D/g,'')" />
            ...
            <?php if(!empty($output['specifications']) && is_array($output['specifications'])){?>
            	...
            	<tr id="spec_<?php echo $k_one;?>" nc_type="good_data">
            		<input type="hidden" name="good_sku" value="<?php echo $k_one;?>" />
            		<?php foreach ($output['specifications_check'] as $k=>$val){?>
            			<input type="hidden" name="spec[<?php echo $k_one;?>][sp_value][]" value="<?php echo $vals[$k];?>" />
            			<td><?php echo $vals[$k];?></td>
            		<?php }?>																													
            		<td align="center"><input style="width:35px;" class="text price" type="text" name="spec[<?php echo $k_one;?>][marketprice]" data_type="marketprice" nc_type="price" value="" pctype="pice_num" /></td>
            		<td align="center"><input style="width:35px;" class="text price" type="text" name="spec[<?php echo $k_one;?>][price]" data_type="price" nc_type="price" value="" pctype="pice_num" /></td>
            		<td align="center"><input style="width:35px;" class="text price" type="text" name="spec[<?php echo $k_one;?>][goods_third_price]" data_type="goods_third_price" nc_type="price" value="" pctype="pice_num" /></td>
            		<td align="center"><input style="width:35px;" class="text price" type="text" name="spec[<?php echo $k_one;?>][cosprice]" data_type="g_costprice" nc_type="price" value="" pctype="pice_num" /></td>
            		<td align="center"><input style="width:35px;" class="text price" type="text" name="spec[<?php echo $k_one;?>][stock]" data_type="stock" nc_type="price" value="" onkeyup="this.value=this.value.replace(/[^0-9]/g,'')"/></td>
            		<td align="center"><input style="width:35px;" class="text price" type="text" name="spec[<?php echo $k_one;?>][alarm]" data_type="alarm" nc_type="price" value="" onkeyup="this.value=this.value.replace(/[^0-9]/g,'')"/></td>
            		<td align="center"><input style="width:35px;" class="text price" type="text" name="spec[<?php echo $k_one;?>][min_num]" data_type="min_num" nc_type="price" value="1" onkeyup="this.value=this.value.replace(/[^0-9]/g,'')"/></td>
            		<td align="center"><input style="width:35px;" class="text price" type="text" name="spec[<?php echo $k_one;?>][max_num]" data_type="max_num" nc_type="price" value="99999" onkeyup="this.value=this.value.replace(/[^0-9]/g,'')"/></td>
            	</tr>
            <?php }}?>
            </tbody>
          </table>
...
$(function(){
	//电脑端手机端tab切换
	$(".tabs").tabs();
        if($("[nc_type='spec_dl']").css("display") == 'none'){
...
/*
            goods_toc_price : {
                required    : '<i class="icon-exclamation-sign"></i>请填写ToC价',
            ...
            goods_third_price : {
                required    : '<i class="icon-exclamation-sign"></i>请填写普通会员价',
                number      : '<i class="icon-exclamation-sign"></i>请填写正确的价格',
                min         : '<i class="icon-exclamation-sign"></i>请填写0.01~9999999之间的数字',
                max         : '<i class="icon-exclamation-sign"></i>请填写0.01~9999999之间的数字'
            }, 
			min_num : {
                ...
                number      : '<i class="icon-exclamation-sign"></i>请填写正确的数量',
                min         : '<i class="icon-exclamation-sign"></i>请填写0~9999999之间的数字',
                max         : '<i class="icon-exclamation-sign"></i>请填写0~9999999之间的数字'
            },
			max_num : {
                required    : '<i class="icon-exclamation-sign"></i>请填写最大购买数量',
                number      : '<i class="icon-exclamation-sign"></i>请填写正确的数量',
...
$(function(){
    ...
    $('input[pctype="pice_num"]').on('keyup', function (event) {
    ...
    $('input[pctype="pice_num"]').on('blur', function () {
        var $amountInput = $(this);
        //最后一位是小数点的话，移除
...
function goods_stock_set(){
    ...
    $('input[name="g_alarm"]').attr('readonly','readonly').css('background','#E7E7E7 none');
    $('input[name="max_num"]').attr('readonly','readonly').css('background','#E7E7E7 none');
    $('input[name="min_num"]').attr('readonly','readonly').css('background','#E7E7E7 none');
    
     $('input[name="max_num"]').val('9999');
     $('input[name="min_num"]').val('0');
    ...
    if(str == '<tr>'){
        ...
        $('input[name="g_alarm"]').removeAttr('readonly').css('background','');
        $('input[name="max_num"]').removeAttr('readonly').css('background','');
        $('input[name="min_num"]').removeAttr('readonly').css('background','');
        ...
        $('tbody[nc_type="spec_table"]').empty().html(str)
            .find('input[nc_type]').each(function(){
                ...
                if ($(this).attr('data_type') == 'max_num' && $(this).val() == ''){
                    $(this).val('9999');
                }
            }).end()
