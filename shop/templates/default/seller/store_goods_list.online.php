...
<table class="ncsc-default-table">
  <thead>
    <tr nc_type="table_header">
      ...
      <th class="w100">修改库存</th>
      <th class="w100">价格库存</th>
      <th class="w100">商品总<?php echo $lang['store_goods_index_stock'];?></th>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td class="tl"><dl class="goods-name">
          ...
          <dd class="serve"> <span class="<?php if ($val['goods_commend'] == 1) { echo 'open';}?>" title="店铺推荐商品"><i class="commend">荐</i></span> <span class="<?php if ($val['mobile_body'] != '') { echo 'open';}?>" title="手机端商品详情"><i class="icon-tablet"></i></span> <span class="" title=""><i class="icon-qrcode"></i>
      ...
      <!--<td><span><?php echo $lang['currency'].$val['goods_price']; ?></span></td>-->
       <td><a data-type="sp_edit" data-value="<?php echo $val['goods_commonid'];?>" data-num="<?php echo $val['sk_num']*90;?>"
      	data-param="{url:'<?php echo urlShop('store_goods_online', 'get_goods_kc_eidt');?>', sign:'plate'}"
      	style="cursor:pointer;"><span>修改库存</span></a></td> 
        
      <td><a data-type="sp_info" data-value="<?php echo $val['goods_commonid'];?>" data-num="<?php echo $val['sk_num']*90;?>"
      	data-param="{url:'<?php echo urlShop('store_goods_online', 'get_good_sp_list');?>', sign:'plate'}"
      	style="cursor:pointer;"><span>点击查看</span></a></td>
...
$(function(){
    ...
    $('a[data-type="sp_info"]').click(function(){
        var w =  parseInt($(this).attr('data-num'))+parseInt('720');
    	var data_str = '';
        eval('data_str = ' + $(this).attr('data-param'));
    	var ajax_url = '';
    	var common = $(this).attr('data-value');
    	ajax_url = data_str.url + '&commonid='+common+'&inajax=1'
        alert(ajax_url);
    	ajax_form('ajax_sp_list', '查看规格库存价格', ajax_url, w);
        //alert(ajax_url);
    	//http://www.wk_line.com/shop/index.php?act=store_goods_online&op=edit_jingle&commonid=5296&inajax=1
    });
    
        $('a[data-type="sp_edit"]').click(function(){
        var w =  parseInt($(this).attr('data-num'))+parseInt('300');
    	var data_str = '';
        eval('data_str = ' + $(this).attr('data-param'));
    	var ajax_url = '';
    	var common = $(this).attr('data-value');
    	ajax_url = data_str.url + '&commonid='+common+'&inajax=1'
    	ajax_form('ajax_sp_list', '修改库存', ajax_url, w);
        //alert(ajax_url);
    	//http://www.wk_line.com/shop/index.php?act=store_goods_online&op=edit_jingle&commonid=5296&inajax=1
    });
    ...
    $('a[nctype="batch"]').click(function(){
        ...
        //伪静态FIX, 判断连接符出错  ? or &
        var ajax_url = '';
        if(data_str.url.indexOf("?")!=-1){
            ajax_url = data_str.url + '&commonid=' + _items + '&inajax=1'
        }else{
            ajax_url = data_str.url + '?commonid=' + _items + '&inajax=1'
        }
