  ...
  <table class="search-form">
    ...
    <tr>
      ...
      <td class="w70 tc"><label class="submit-border">
          <input type="submit" class="submit" id="search" value="<?php echo $lang['store_order_search'];?>" />
        </label></td>
    </tr>
    <tr>
    </tr>
    ...
    <div style="text-align:right;">
        <a class="btns" href="javascript:void(0);" id="ncexport"><span><strong>导出订单Excel</strong></span></a>&nbsp&nbsp&nbsp&nbsp&nbsp
        <a class="btns" href="javascript:void(0);" id="ncexportList"><span><strong>详细订单Excel</strong></span></a>
    </div>
...
<table class="ncsc-default-table order">
  ...
  <tbody>
    ...
    <tr>
        <th colspan="20"><span class="ml10"><?php echo $lang['store_order_order_sn'].$lang['nc_colon'];?>
                <em>
                    <?php if($order['order_num_sequence']){echo $order['mother_orderid'].'-'.$order['order_num_sequence'];}else{?>
...
<span class="fr mr5"> <a href="index.php?act=store_order_detailed&order_id=<?php echo $order_id;?>" class="ncsc-btn-mini" target="_blank" title="打印交易订单"/><i class="icon-print"></i>打印交易订单</a></span>
<span class="fr mr5"> <a href="index.php?act=store_order_print&order_id=<?php echo $order_id;?>" class="ncsc-btn-mini" target="_blank" title="打印发货单"/><i class="icon-print"></i>打印发货单</a></span>
    ...
    <tr>
      ...
      <td class="tl"><dl class="goods-name">
          <dt><a target="_blank" href="<?php echo $goods['goods_url'];?>" title='<?php echo $goods['goods_name']; ?>'><?php echo $goods['goods_name']; ?></a></dt>
          <dd>
      ...
      <td class="bdl bdr" rowspan="<?php echo $order['goods_count'];?>"><p><?php echo $order['state_desc']; ?> </p>
        
        <!-- 订单查看 -->
        <p><a href="index.php?act=store_transport_nei&op=show_order&order_id=<?php echo $order_id;?>" target="_blank"><?php echo $lang['store_order_view_order'];?></a></p>
      ...
      <?php if($order['order_state'] == ORDER_STATUS_SEND_TWO){?>
          <p><?php echo $lang['pending_audit'];?></p>
      <?php }else if($order['order_state'] == ORDER_STATUS_SEND_ONE){?>
          <p><?php echo $lang['order_audit'];?></p>
      <!-- 待受理 -->
      <?php }else if($order['order_state'] == ORDER_STATUS_SUCCESS){?>
          <p><?php echo $lang['acceptance_audit'];?></p>
      <!-- *********** -->
      <!-- 待取消 -->
      <?php }else if($order['order_state'] == ORDER_WAIT_CANCEL){?>
          <p><?php echo $lang['order_wait_cancel'];?></p>
      <!-- *********** -->
      <!-- 已经取消 -->
      <?php }else if($order['order_state'] == ORDER_STATE_CANCEL){?>
          <p><?php echo $lang['order_cancel'];?></p>
      <!-- *********** -->
      <!-- 待发货 -->
      <?php }else if($order['order_state'] == ORDER_DELIVER_GOODS){?>
      	  <p><?php echo $lang['deliver_audit'];?></p>
      <!-- ********** -->
      <?php }else if($order['order_state'] == ORDER_STATUS_ERROR){?>
          <p><?php echo $lang['audit_return'];?></p>
      <?php }else if($order['order_state'] == ORDER_STATUS_OUT){?>
          <p><?php echo $lang['audit_reject'];?></p>
      <?php }else if($order['order_state'] == ORDER_STATUS_SEND_HET){?>
          <p><?php echo $lang['audit_insend'];?></p>
      <?php }else if($order['order_state'] == ORDER_STATUS_RECEIVED){?>
          <p><?php echo $lang['audit_success'];?></p>
      <?php }else if($order['order_state'] == ORDER_STATUS_CUS_RECEIVED){?>
          <p><?php echo $lang['audit_copyleng'];?></p>
      <?php }?>
          
          <?php if($order['lock_state'] != 1){?>
        <!-- 取消订单 -->
        <?php if($order['order_state'] == ORDER_STATUS_SEND_ONE) { ?>
        <p><a href="javascript:void(0)" class="ncsc-btn ncsc-btn-red mt5" nc_type="dialog" uri="index.php?act=store_transport_nei&op=change_state&state_type=order_cancel&order_sn=<?php echo $order['order_sn']; ?>&order_id=<?php echo $order['order_id']; ?>" dialog_title="<?php echo $lang['store_order_cancel_order'];?>" dialog_id="seller_order_cancel_order" dialog_width="400" id="order<?php echo $order['order_id']; ?>_action_cancel" /><i class="icon-remove-circle"></i><?php echo $lang['store_order_cancel_order'];?></a></p>
        <?php } ?>
        
        <!-- 修改运费 -->
        <?php if ($order['order_state'] == ORDER_STATUS_ERROR || $order['order_state'] == ORDER_STATUS_SEND_ONE) { ?>
        <p><a href="javascript:void(0)" class="ncsc-btn-mini ncsc-btn-orange mt10" uri="index.php?act=store_transport_nei&op=change_state&state_type=modify_price&order_sn=<?php echo $order['order_sn']; ?>&order_id=<?php echo $order['order_id']; ?>" dialog_width="480" dialog_title="<?php echo $lang['store_order_modify_price'];?>" nc_type="dialog"  dialog_id="seller_order_adjust_fee" id="order<?php echo $order['order_id']; ?>_action_adjust_fee" /><i class="icon-pencil"></i>修改运费</a></p>
        <?php }?>
        <!-- 修改价格 -->
		<?php if ($order['order_state'] == ORDER_STATUS_ERROR || $order['order_state'] == ORDER_STATUS_SEND_ONE) { ?>
        <p><a href="javascript:void(0)" class="ncsc-btn-mini ncsc-btn-green mt10" uri="index.php?act=store_transport_nei&op=change_state&state_type=spay_price&order_sn=<?php echo $order['order_sn']; ?>&order_id=<?php echo $order['order_id']; ?>" dialog_width="480" dialog_title="<?php echo $lang['store_order_modify_price'];?>" nc_type="dialog"  dialog_id="seller_order_adjust_fee" id="order<?php echo $order['order_id']; ?>_action_adjust_fee" /><i class="icon-pencil"></i>修改价格</a></p>
		<?php }?>
          <?php }?>
        <!-- 发货 -->
        <?php if ($order['order_state'] == ORDER_DELIVER_GOODS && !$order['if_lock']) { ?>
        ...
        <?php } ?>
        <!-- 受理 -->
        <?php if ($order['order_state'] == ORDER_STATUS_SUCCESS && !$order['if_lock']) { ?>
        <p><a href="javascript:void(0)" class="ncsc-btn-mini ncsc-btn-green mt10" uri="index.php?act=store_transport_nei&op=change_state&state_type=accept&order_sn=<?php echo $order['order_sn']; ?>&order_id=<?php echo $order['order_id']; ?>" dialog_width="480" dialog_title="受理订单" nc_type="dialog"  dialog_id="seller_order_adjust_fee" id="order<?php echo $order['order_id']; ?>_action_adjust_fee" /><i class="icon-pencil"></i>接收订单</a></p>
        <?php } ?>
        
        <!-- 待取消订单 -->
        <?php if($order['order_state'] == ORDER_WAIT_CANCEL) { ?>
        <p><a style="background-color:#FF9797" href="javascript:void(0)" class="ncsc-btn ncsc-btn-red mt5" nc_type="dialog" uri="index.php?act=store_transport_nei&op=change_state&state_type=order_cancel&order_sn=<?php echo $order['order_sn']; ?>&order_id=<?php echo $order['order_id']; ?>" dialog_title="<?php echo $lang['store_order_cancel_order'];?>" dialog_id="seller_order_cancel_order" dialog_width="400" id="order<?php echo $order['order_id']; ?>_action_cancel" /><i class="icon-remove-circle"></i>同意取消订单</a></p>
        <p><a href="javascript:void(0)" class="ncsc-btn ncsc-btn-red mt5" nc_type="dialog" uri="index.php?act=store_transport_nei&op=refuseOrder&order_sn=<?php echo $order['order_sn']; ?>&order_id=<?php echo $order['order_id']; ?>" dialog_title="<?php echo $lang['store_order_cancel_order'];?>" dialog_id="seller_order_cancel_order" dialog_width="400" id="order<?php echo $order['order_id']; ?>_action_cancel" /><i class="icon-remove-circle"></i>拒绝取消订单</a></p>
            <?php } ?>
...
$(function(){
    $('#ncexportList').click(function(){
    $('input[name="op"]').val('Purchaseexport_step2');
    $('#formSearch').submit();
    });
});
</script>
