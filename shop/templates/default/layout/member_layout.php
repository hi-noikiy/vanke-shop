...
<div class="ncm-container">
  <div class="left-layout">
    <div class="ncm-l-top">
      <h2><a href="<?php echo urlShop('member', 'home');?>">我的商城</a></h2>
    ...
    <div class="ncm-user-info">
      <div class="avatar"><img src="<?php echo getMemberAvatar($output['member_info']['member_avatar']);?>">
        ...
        <a href="<?php echo urlShop('member_message', 'message');?>" class="new-message" title="新消息"><?php echo intval($output['message_num']); ?></a>
        <?php }?>
      </div>
      <div class="handle">
<!--          --><?php //if( $_SESSION['identity'] == MEMBER_IDENTITY_TWO ){?>
<!--          <a href="javarscript:viod(0);" style="color: #7aa61b;font-weight: 600;border-bottom: 1px solid #4c6f02;" class="open_listca" title="采购后台"><i style="font-size: 16px;" class="icon-purchase" ></i>采购后台</a>-->
<!--          --><?php //} ?>
          <a href="<?php echo urlShop('member_information', 'avatar');?>" title="修改头像"><i class="icon-camera"></i>修改头像</a><a href="<?php echo urlShop('member_information', 'member');?>" title="修改资料"><i class="icon-pencil"></i>修改资料</a><a href="<?php echo urlShop('login', 'logout');?>" title="安全退出"><i class="icon-off"></i>安全退出</a></div>
...
<!--        <div class="nc-grade-mini" style="cursor:pointer;" onclick="javascript:go('<?php //echo urlShop('pointgrade','index');?>//');"><?php //echo $output['member_info']['level_name'];?></div>-->
      ...
      <div class="btnCome">
          <?php if( $_SESSION['identity'] == MEMBER_IDENTITY_TWO ){
              echo'<a href="'.YMA_IFRAME_URL_HEAD ?>/impac/loginFromUrl.do?shopCd=<?php echo Embedpage::getPurShopCdByDES();?>&userCd=<?php echo Embedpage::getPurUserCdByDES( $output['member_info']['pernr_id'])?>&validateFlg=<?php echo 'true'.'" >采购后台</a>';
           }
          else if($_SESSION['identity'] == MEMBER_IDENTITY_THREE) {
              echo '<a href="'. urlShop('show_join','index') .'" >商家入驻</a>';
              ;
          }else if($_SESSION['identity'] == MEMBER_IDENTITY_FOUR){
              echo '<a href="'. urlShop('seller_login','show_login') .'">商家中心</a>';
              ;
...
<script>
    $(function(){
        $('.open_listca').click(function(){
            if ( '<?php if( $_SESSION['identity'] == MEMBER_IDENTITY_TWO ) echo 'true';else echo"false"; ?>' === 'true' ) {
                window.open("<?php echo YMA_IFRAME_URL_HEAD ?>/impac/loginFromUrl.do?shopCd=<?php echo Embedpage::getPurShopCdByDES();?>&userCd=<?php echo Embedpage::getPurUserCdByDES( $output['member_info']['pernr_id']);?>");
              }else{
