...
if(($ua == '' || preg_match($uachar, $ua))&& !strpos(strtolower($_SERVER['REQUEST_URI']),'wap') && empty($_SERVER['QUERY_STRING']))
...
<link href="<?php echo SHOP_RESOURCE_SITE_URL;?>/font/font-awesome/css/font-awesome.min.css" rel="stylesheet" />
    ...
    <link href="<?php echo SHOP_TEMPLATES_URL;?>/css/style1.css" rel="stylesheet" type="text/css">
    <?php //} ?>
<!--[if IE 7]>
...
$(function(){
    ...
    //采购学堂
    $('#pur_school').click(function(){
        window.location.href=("<?php echo SHOP_SITE_URL;?>/index.php?act=article&op=purchase_school");
    });
    $('#tender_info').click(function(){
...
<!--            <div class="nc-grade-mini" style="cursor:pointer;" onClick="javascript:go('<?php //echo urlShop('pointgrade','index');?>');"><?php //echo $output['member_info']['level_name'];?></div>-->
          ...
          <div class="user-centent-menu">
            <ul>
              <li><a href="<?php echo urlShop('member_message','message');?>">站内消息(<span><?php echo $output['message_num']>0 ? $output['message_num']:'0';?></span>)</a></li>
              <?php if( $_SESSION['identity'] == MEMBER_IDENTITY_TWO ){
              ?>
              <li><a href="<?php echo urlShop('member_inorder', 'inside_order');?>" class="arrow">订单查询<i></i></a></li>
              <?php
                }
               else if($_SESSION['identity'] == MEMBER_IDENTITY_THREE) {
                   ?>
                   <li><a href="<?php echo urlShop('member_unline', 'index');?>" class="arrow">线下订单<i></i></a></li>
                   <?php
                   ...
                   ?>
                   <li><a href="<?php echo urlShop('member_order_third', 'index');?>" class="arrow">订单查询<i></i></a></li>
                   <?php
               }?>
              
              <li><a href="<?php echo urlShop('member_consult','my_consult');?>">咨询回复(<span id="member_consult">0</span>)</a></li>
              <li><a href="<?php echo urlShop('member_favorites','fglist');?>" class="arrow">我的收藏<i></i></a></li>
              <?php /*if (C('voucher_allow') == 1){*/?><!--
              <li><a href="<?php /*echo urlShop('member_voucher','');*/?>">代金券(<span id="member_voucher">0</span>)</a></li>
              <?php /*} */?>
              <?php /*if (C('points_isuse') == 1){ */?>
              <li><a href="<?php /*echo urlShop('member_points','');*/?>" class="arrow">我的积分<i></i></a></li>
              --><?php /*} */?>
            </ul>
          </div>
          <div class="browse-history">
            <div class="part-title">
              <h4>最近浏览的商品</h4>
              <span style="float:right;"><a href="<?php echo urlShop('member_goodsbrowse','list');?>">全部浏览历史</a></span>
            </div>
            <ul>
      ...
      <dl class="my-cart">
        ...
        <?php } ?>
        <dt><span class="ico"></span>购物车<i class="arrow"></i></dt>
        <dd>
          <div class="sub-title">
            <h4>最新加入的商品</h4>
          </div>
          ...
          <div class="checkout"> <span class="total-price">共<i><?php echo $output['cart_goods_num'];?></i><?php echo $lang['nc_kindof_goods'];?></span><a href="<?php echo urlShop('cart','');?>" class="btn-cart">结算购物车中的商品</a> </div>
        </dd>
      </dl>
    </div>
    <?php }?>
...
<!--        --><?php //} ?>
      <li><a href="javascript:void(0);" id="pur_school"  <?php if($_GET['op'] == 'purchase_school') {echo 'class="current"';} ?>>采购学堂</a></li>

        <?php if (C('flea_isuse') && false){ ?>
      <li><a href="<?php echo urlShop('flea', 'index');?>" <?php if($output['index_sign'] == 'flea' && $output['index_sign'] != '0') {echo 'class="current"';} ?>> 闲置</a></li>
      <?php } ?>

        <?php if (C('cms_isuse') && false){ ?>
