...
//         $input->SetBody(C('site_name').'订单');
        ...
        //add cary
        $AttachStr = 'r';
        switch($this->_order_info['order_type']){
            case 'real_order': $AttachStr= 'r';break;
            case 'vr_order': $AttachStr= 'v';break;
            case 'pd_order': $AttachStr = 'p';break;
        }

//        $input->SetAttach($this->_order_info['order_type'] == 'vr_order' ? 'v' : 'r');
        $input->SetAttach($AttachStr);
...
file_put_contents('/alidata/www/default/data/log/wxpay/debug.log','debug'.json_encode($result), FILE_APPEND );
