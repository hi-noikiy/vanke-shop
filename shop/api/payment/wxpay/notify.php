	...
	public function NotifyProcess($data, &$msg)
	{
		...
		if (in_array($data['attach'],array('v','r','p'))) {
		    ...
		    if(empty($order_pay_info)){
				...
				// todo 增加vr和充值判断
//				$vr_order_pay_info = Model('vr_order')->getOrderInfo(array('pay_sn'=> $data['out_trade_no']));
		...
		if ($order_type == 'r') {
			...
			$order_info = $result['data'];
			$payment_info =array();
			$payment_info['payment_code'] = 'wx_saoma';
			$payment_info['payment_name'] = '微信扫码';
		...
		if ($order_type == 'r') {
			...
			$log_buyer_id = $order_info['buyer_id'];
			$log_buyer_name = $order_info['buyer_name'];
			$log_desc = '余额充值订单使用微信扫码成功支付，支付单号：'.$data['out_trade_no'];
		}
