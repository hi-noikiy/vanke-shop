...
class alipay{
    ...
    public function get_payurl(){
        ...
        $arr['service']             = $this->parameter['service'];
        $arr['_input_charset']      = $this->parameter['_input_charset'];
        $arr['notify_url']          = $this->parameter['return_url'];
        $arr['return_url']          = $this->parameter['return_url'];
        $arr['extra_common_param']  = $this->parameter['extra_common_param'];
        $arr['subject']             = $this->parameter['subject'];
        $arr['body']                = $this->parameter['body'];
        $arr['out_trade_no']        = $this->parameter['out_trade_no'];
        $arr['payment_type']        = (string)$this->parameter['payment_type'];
        $arr['total_fee']           = (string)$this->parameter['price'];
        $arr['extend_param']        = $this->parameter['extend_param'];
        $get_key_url = 'http://10.0.57.114:9001/vkwyinterface/order/fastpay/sign';
        
        $key="0978falconuipcross12wer4ct47X";
        ksort($arr);
        foreach($arr as $k => $v){
                $link.="$k=$v"."&";
        }
        $sign=md5(substr($link,0,-1).$key);
        $arr['sign']=$sign;
        $arr['sign_type']='md5';

        $url = $this->alipay_post_submit(json_encode($arr), $get_key_url);
        $array_url = json_decode($url,true);
        if($array_url['process_status'] == 'success'){
            $re_cpay_url = $this->alipay_gateway_new.$array_url['process_ret'];
            return $re_cpay_url;
        }
...
//        return $this->create_url();
    ...
    public function alipay_post_submit($data,$url){  
//        $o="";  
//        foreach ($data as $k=>$v)  
//        {  
//            $o.= "$k=".urlencode($v)."&";  
//        }  
//        $re_data=substr($o,0,-1);  
//        $ch = curl_init();  
